'use client';

import { useState, useEffect, useRef } from 'react';
import HorizontalScroll from '@/components/HorizontalScroll';
import Card from '@/components/Card';
import ProgressIndicator from '@/components/ProgressIndicator';

export default function Home() {
  const [scrollProgress, setScrollProgress] = useState(0);
  const [isOutlineMode, setIsOutlineMode] = useState(false);
  const [scrambledText, setScrambledText] = useState('');
  const [card2ScrambledText, setCard2ScrambledText] = useState('');
  const [card2HasBeenViewed, setCard2HasBeenViewed] = useState(false);
  const [card3ScrambledText, setCard3ScrambledText] = useState('');
  const [card3HasBeenViewed, setCard3HasBeenViewed] = useState(false);
  const [card4ScrambledText, setCard4ScrambledText] = useState('');
  const [card4HasBeenViewed, setCard4HasBeenViewed] = useState(false);
  const [card5ScrambledText, setCard5ScrambledText] = useState('');
  const [card5HasBeenViewed, setCard5HasBeenViewed] = useState(false);
  const [card6ScrambledText, setCard6ScrambledText] = useState('');
  const [card6HasBeenViewed, setCard6HasBeenViewed] = useState(false);
  const [card7ScrambledText, setCard7ScrambledText] = useState('');
  const [card7HasBeenViewed, setCard7HasBeenViewed] = useState(false);

  // Contact form state
  const [formName, setFormName] = useState('');
  const [formEmail, setFormEmail] = useState('');
  const [formMessage, setFormMessage] = useState('');
  const [formLoading, setFormLoading] = useState(false);
  const [formSuccess, setFormSuccess] = useState(false);
  const [formError, setFormError] = useState('');

  const card2Ref = useRef<HTMLDivElement>(null);
  const card3Ref = useRef<HTMLDivElement>(null);
  const card4Ref = useRef<HTMLDivElement>(null);
  const card5Ref = useRef<HTMLDivElement>(null);
  const card6Ref = useRef<HTMLDivElement>(null);
  const card7Ref = useRef<HTMLDivElement>(null);

  const finalText = 'John Zemrose is a Systems Architect building AI-native engineering organizations through servant leadership and deep technical fluency';
  const card2FinalText = 'AI-Native Engineering';
  const card3FinalText = 'Machine Learning';
  const card4FinalText = 'Data Engineering';
  const card5FinalText = 'Business Intelligence';
  const card6FinalText = 'About';
  const card7FinalText = "Let's Connect";
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*';

  // Contact form submission handler
  const handleContactSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormLoading(true);
    setFormError('');
    setFormSuccess(false);

    try {
      const response = await fetch('/api/contact', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: formName,
          email: formEmail,
          message: formMessage,
        }),
      });

      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.error || 'Failed to send message');
      }

      setFormSuccess(true);
      setFormName('');
      setFormEmail('');
      setFormMessage('');
    } catch (error) {
      setFormError(error instanceof Error ? error.message : 'Something went wrong');
    } finally {
      setFormLoading(false);
    }
  };

  useEffect(() => {
    let currentIndex = 0;
    let scrambleCount = 0;
    const scrambleIterations = 2; // How many times each character scrambles

    const interval = setInterval(() => {
      setScrambledText(
        finalText
          .split('')
          .map((char, index) => {
            if (index > currentIndex) return '';
            if (char === ' ') return ' ';
            if (index < currentIndex) return char;

            // Current character being "typed" - scramble it a few times
            if (scrambleCount < scrambleIterations) {
              return chars[Math.floor(Math.random() * chars.length)];
            }
            return char;
          })
          .join('')
      );

      scrambleCount++;

      if (scrambleCount >= scrambleIterations) {
        currentIndex++;
        scrambleCount = 0;
      }

      if (currentIndex >= finalText.length) {
        clearInterval(interval);
        setScrambledText(finalText);
      }
    }, 10);

    return () => clearInterval(interval);
  }, []);

  // Card 2 scroll-triggered scramble animation
  useEffect(() => {
    if (!card2Ref.current || card2HasBeenViewed) return;

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !card2HasBeenViewed) {
            setCard2HasBeenViewed(true);

            // Start scramble animation
            let currentIndex = 0;
            let scrambleCount = 0;
            const scrambleIterations = 2;

            const interval = setInterval(() => {
              setCard2ScrambledText(
                card2FinalText
                  .split('')
                  .map((char, index) => {
                    if (index > currentIndex) return '';
                    if (char === ' ' || char === '-') return char;
                    if (index < currentIndex) return char;

                    if (scrambleCount < scrambleIterations) {
                      return chars[Math.floor(Math.random() * chars.length)];
                    }
                    return char;
                  })
                  .join('')
              );

              scrambleCount++;

              if (scrambleCount >= scrambleIterations) {
                currentIndex++;
                scrambleCount = 0;
              }

              if (currentIndex >= card2FinalText.length) {
                clearInterval(interval);
                setCard2ScrambledText(card2FinalText);
              }
            }, 10);
          }
        });
      },
      {
        threshold: 0.5, // Trigger when 50% of the card is visible
      }
    );

    observer.observe(card2Ref.current);

    return () => {
      if (card2Ref.current) {
        observer.unobserve(card2Ref.current);
      }
    };
  }, [card2HasBeenViewed]);

  // Card 3 scroll-triggered scramble animation
  useEffect(() => {
    if (!card3Ref.current || card3HasBeenViewed) return;

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !card3HasBeenViewed) {
            setCard3HasBeenViewed(true);
            let currentIndex = 0;
            let scrambleCount = 0;
            const scrambleIterations = 2;

            const interval = setInterval(() => {
              setCard3ScrambledText(
                card3FinalText.split('').map((char, index) => {
                  if (index > currentIndex) return '';
                  if (char === ' ' || char === '-') return char;
                  if (index < currentIndex) return char;
                  if (scrambleCount < scrambleIterations) {
                    return chars[Math.floor(Math.random() * chars.length)];
                  }
                  return char;
                }).join('')
              );
              scrambleCount++;
              if (scrambleCount >= scrambleIterations) {
                currentIndex++;
                scrambleCount = 0;
              }
              if (currentIndex >= card3FinalText.length) {
                clearInterval(interval);
                setCard3ScrambledText(card3FinalText);
              }
            }, 10);
          }
        });
      },
      { threshold: 0.5 }
    );

    observer.observe(card3Ref.current);
    return () => {
      if (card3Ref.current) {
        observer.unobserve(card3Ref.current);
      }
    };
  }, [card3HasBeenViewed]);

  // Card 4 scroll-triggered scramble animation
  useEffect(() => {
    if (!card4Ref.current || card4HasBeenViewed) return;

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !card4HasBeenViewed) {
            setCard4HasBeenViewed(true);
            let currentIndex = 0;
            let scrambleCount = 0;
            const scrambleIterations = 2;

            const interval = setInterval(() => {
              setCard4ScrambledText(
                card4FinalText.split('').map((char, index) => {
                  if (index > currentIndex) return '';
                  if (char === ' ' || char === '-') return char;
                  if (index < currentIndex) return char;
                  if (scrambleCount < scrambleIterations) {
                    return chars[Math.floor(Math.random() * chars.length)];
                  }
                  return char;
                }).join('')
              );
              scrambleCount++;
              if (scrambleCount >= scrambleIterations) {
                currentIndex++;
                scrambleCount = 0;
              }
              if (currentIndex >= card4FinalText.length) {
                clearInterval(interval);
                setCard4ScrambledText(card4FinalText);
              }
            }, 10);
          }
        });
      },
      { threshold: 0.5 }
    );

    observer.observe(card4Ref.current);
    return () => {
      if (card4Ref.current) {
        observer.unobserve(card4Ref.current);
      }
    };
  }, [card4HasBeenViewed]);

  // Card 5 scroll-triggered scramble animation
  useEffect(() => {
    if (!card5Ref.current || card5HasBeenViewed) return;

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !card5HasBeenViewed) {
            setCard5HasBeenViewed(true);
            let currentIndex = 0;
            let scrambleCount = 0;
            const scrambleIterations = 2;

            const interval = setInterval(() => {
              setCard5ScrambledText(
                card5FinalText.split('').map((char, index) => {
                  if (index > currentIndex) return '';
                  if (char === ' ' || char === '-') return char;
                  if (index < currentIndex) return char;
                  if (scrambleCount < scrambleIterations) {
                    return chars[Math.floor(Math.random() * chars.length)];
                  }
                  return char;
                }).join('')
              );
              scrambleCount++;
              if (scrambleCount >= scrambleIterations) {
                currentIndex++;
                scrambleCount = 0;
              }
              if (currentIndex >= card5FinalText.length) {
                clearInterval(interval);
                setCard5ScrambledText(card5FinalText);
              }
            }, 10);
          }
        });
      },
      { threshold: 0.5 }
    );

    observer.observe(card5Ref.current);
    return () => {
      if (card5Ref.current) {
        observer.unobserve(card5Ref.current);
      }
    };
  }, [card5HasBeenViewed]);

  // Card 6 scroll-triggered scramble animation
  useEffect(() => {
    if (!card6Ref.current || card6HasBeenViewed) return;

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !card6HasBeenViewed) {
            setCard6HasBeenViewed(true);
            let currentIndex = 0;
            let scrambleCount = 0;
            const scrambleIterations = 2;

            const interval = setInterval(() => {
              setCard6ScrambledText(
                card6FinalText.split('').map((char, index) => {
                  if (index > currentIndex) return '';
                  if (char === ' ' || char === '-') return char;
                  if (index < currentIndex) return char;
                  if (scrambleCount < scrambleIterations) {
                    return chars[Math.floor(Math.random() * chars.length)];
                  }
                  return char;
                }).join('')
              );
              scrambleCount++;
              if (scrambleCount >= scrambleIterations) {
                currentIndex++;
                scrambleCount = 0;
              }
              if (currentIndex >= card6FinalText.length) {
                clearInterval(interval);
                setCard6ScrambledText(card6FinalText);
              }
            }, 10);
          }
        });
      },
      { threshold: 0.5 }
    );

    observer.observe(card6Ref.current);
    return () => {
      if (card6Ref.current) {
        observer.unobserve(card6Ref.current);
      }
    };
  }, [card6HasBeenViewed]);

  // Card 7 scroll-triggered scramble animation
  useEffect(() => {
    if (!card7Ref.current || card7HasBeenViewed) return;

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !card7HasBeenViewed) {
            setCard7HasBeenViewed(true);
            let currentIndex = 0;
            let scrambleCount = 0;
            const scrambleIterations = 2;

            const interval = setInterval(() => {
              setCard7ScrambledText(
                card7FinalText.split('').map((char, index) => {
                  if (index > currentIndex) return '';
                  if (char === ' ' || char === '-') return char;
                  if (char === "'") return char;
                  if (index < currentIndex) return char;
                  if (scrambleCount < scrambleIterations) {
                    return chars[Math.floor(Math.random() * chars.length)];
                  }
                  return char;
                }).join('')
              );
              scrambleCount++;
              if (scrambleCount >= scrambleIterations) {
                currentIndex++;
                scrambleCount = 0;
              }
              if (currentIndex >= card7FinalText.length) {
                clearInterval(interval);
                setCard7ScrambledText(card7FinalText);
              }
            }, 10);
          }
        });
      },
      { threshold: 0.5 }
    );

    observer.observe(card7Ref.current);
    return () => {
      if (card7Ref.current) {
        observer.unobserve(card7Ref.current);
      }
    };
  }, [card7HasBeenViewed]);

  return (
    <main className="min-h-screen relative">
      <HorizontalScroll onScroll={setScrollProgress}>
        {/* Card 1: Home/Intro */}
        <Card isOutlineMode={isOutlineMode}>
          <div
            className="h-full flex items-center"
            style={{
              paddingLeft: '48px'
            }}
          >
            {/* Left-aligned content - large bold text */}
            <div className="relative z-10 max-w-[400px]">
              <h1
                className="text-[3rem] font-light leading-[1.1] tracking-tight"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                {scrambledText || finalText}
              </h1>
            </div>

            {/* Right-side visual element - burgundy circle (touches edges) */}
            <div
              className="absolute"
              style={{
                width: '525px',
                height: '525px',
                borderRadius: '50%',
                backgroundColor: isOutlineMode ? 'transparent' : 'var(--color-3)',
                border: isOutlineMode ? '1px solid var(--text-secondary)' : 'none',
                right: '0',
                top: '0',
              }}
            />
          </div>
        </Card>

        {/* Card 2: AI-Native Engineering */}
        <Card href="/ai-native" isOutlineMode={isOutlineMode}>
          <div ref={card2Ref} className="h-full flex flex-col relative overflow-hidden" style={{ padding: '60px' }}>
            {/* Tech Stack Logos - vertically stacked, right aligned */}
            <div
              className="absolute flex flex-col gap-6 items-end"
              style={{
                right: '80px',
                top: '50%',
                transform: 'translateY(-50%)',
                paddingRight: '16px',
              }}
            >
              {/* Claude Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="100%" height="100%">
                  <g fill="#534c44" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" style={{mixBlendMode: 'normal'}}>
                    <g transform="scale(5.12,5.12)">
                      <path d="M19.861,27.625v-0.716l-16.65,-0.681l-1.141,-0.243l-1.07,-1.41l0.11,-0.703l0.959,-0.645l17.95,1.345l0.11,-0.314l-14.413,-9.893l-0.729,-0.924l-0.314,-2.016l1.312,-1.445l2.214,0.24l11.312,8.602l0.327,-0.353l-7.215,-12.492c0,0 -0.548,-2.175 -0.548,-2.697l1.494,-2.029l0.827,-0.266l2.833,0.995l7.935,17.331h0.314l1.348,-14.819l0.752,-1.822l1.494,-0.985l1.167,0.557l0.959,1.374l-2.551,14.294h0.425l0.486,-0.486l8.434,-10.197l1.092,-0.862h2.065l1.52,2.259l-0.681,2.334l-7.996,11.108l0.146,0.217l0.376,-0.036l12.479,-2.405l1.666,0.778l0.182,0.791l-0.655,1.617l-15.435,3.523l-0.084,0.062l0.097,0.12l13.711,0.814l1.578,1.044l0.947,1.277l-0.159,0.972l-2.431,1.238l-13.561,-3.254h-0.363v0.217l11.218,10.427l0.256,1.154l-0.645,0.911l-0.681,-0.097l-9.967,-8.058h-0.256v0.34l5.578,8.35l0.243,2.162l-0.34,0.703l-1.215,0.425l-1.335,-0.243l-7.863,-12.083l-0.279,0.159l-1.348,14.524l-0.632,0.742l-1.459,0.558l-1.215,-0.924l-0.646,-1.494l2.966,-14.939l-0.023,-0.084l-0.279,0.036l-10.683,13.528l-0.827,0.327l-1.433,-0.742l0.133,-1.326l0.801,-1.18l9.52,-12.019l-0.013,-0.314h-0.11l-12.69,8.239l-2.259,0.292l-0.973,-0.91l0.12,-1.494l0.46,-0.486z"></path>
                    </g>
                  </g>
                </svg>
              </div>

              {/* Turborepo Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 318" width="100%" height="100%">
                  <path fill="#534c44" d="M103.41.095C66.837-1.166 30.268 10.183 0 34.144l20.177 11.35c23.962-17.656 54.226-25.222 83.233-23.961z"/>
                  <path fill="#534c44" d="M210.6 48.016c-25.222-26.483-58.01-42.877-93.319-46.66v21.438c30.268 5.044 56.75 18.916 78.188 40.355z"/>
                  <path fill="#534c44" d="M256 151.424c-1.26-32.788-13.871-65.576-35.309-93.32l-15.136 15.133c17.657 23.96 27.747 50.444 29.008 78.187z"/>
                  <path fill="#534c44" d="M220.691 259.877c21.438-27.744 34.049-60.532 35.309-93.32h-21.437c-1.26 27.744-11.351 54.227-29.008 78.187z"/>
                  <path fill="#534c44" d="M117.281 316.626c34.049-3.783 68.097-20.178 93.32-46.66l-15.132-15.133c-21.437 22.7-49.185 36.571-78.188 40.354z"/>
                  <path fill="#534c44" d="M0 283.838c30.268 23.96 66.837 35.31 103.41 34.049v-21.439c-29.007 1.262-58.01-6.305-83.233-23.96z"/>
                  <path fill="#534c44" d="M97.651 71.344c13.479 0 25.919 2.295 37.321 6.886c11.402 4.443 21.177 10.663 29.324 18.66c8.292 7.998 14.734 17.402 19.326 28.214c4.593 10.663 6.887 22.214 6.887 34.655c0 12.44-2.294 24.066-6.887 34.877c-4.592 10.663-11.034 19.993-19.326 27.99c-8.147 7.998-17.922 14.292-29.324 18.883c-11.402 4.443-23.842 6.665-37.32 6.665c-13.625 0-26.142-2.222-37.544-6.665c-11.257-4.591-20.954-10.885-29.101-18.882c-8.147-7.998-14.513-17.328-19.105-27.991c-4.592-10.811-6.887-22.437-6.887-34.877c0-12.44 2.295-23.992 6.887-34.655c4.592-10.812 10.958-20.216 19.105-28.213c8.147-7.998 17.844-14.218 29.101-18.66c11.402-4.592 23.92-6.887 37.543-6.887m0 42.208c-6.37 0-12.44 1.184-18.216 3.554c-5.626 2.222-10.59 5.406-14.884 9.553c-4.293 4.146-7.702 9.034-10.219 14.661c-2.516 5.628-3.776 11.774-3.776 18.439c0 6.664 1.26 12.81 3.776 18.438c2.517 5.628 5.926 10.515 10.22 14.662c4.293 4.147 9.257 7.404 14.883 9.774c5.776 2.222 11.846 3.332 18.216 3.332c6.37 0 12.368-1.11 17.994-3.332c5.776-2.37 10.813-5.627 15.106-9.774c4.443-4.147 7.925-9.034 10.441-14.662c2.516-5.628 3.777-11.774 3.777-18.438c0-6.665-1.26-12.811-3.777-18.439c-2.516-5.627-5.998-10.515-10.44-14.661c-4.294-4.147-9.331-7.331-15.107-9.553c-5.626-2.37-11.624-3.554-17.994-3.554"/>
                </svg>
              </div>

              {/* Docker Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="100%" height="100%">
                  <g fill="#534c44" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" style={{mixBlendMode: 'normal'}}>
                    <g transform="scale(10.66667,10.66667)">
                      <path d="M10.64453,3.5v2.30273h2.47656v-2.30273zM4.78711,6.25781v2.2793h2.47852v-2.2793zM7.7168,6.25781v2.2793h2.47656v-2.2793zM10.64453,6.25781v2.2793h2.47656v-2.2793zM18.69531,6.96289l-0.34961,0.53125c-0.69,1.055 -0.87767,2.7915 -0.13867,3.9375c-0.326,0.178 -0.96645,0.42125 -1.81445,0.40625h-16.29883c-0.326,1.927 0.21644,4.43139 1.64844,6.15039c1.39,1.666 3.47517,2.51172 6.20117,2.51172c5.902,0 10.27045,-2.74909 12.31445,-7.74609c0.802,0.015 2.53483,0.0022 3.42383,-1.7168c0.056,-0.095 0.24736,-0.50044 0.31836,-0.64844l-0.49805,-0.33789v0.00195c-0.54,-0.368 -1.78423,-0.50231 -2.74023,-0.32031c-0.124,-0.911 -0.62602,-1.70302 -1.54102,-2.41602zM1.86133,8.99414v2.25195h2.47852v-2.25195h-0.00195zM4.78711,8.99414v2.25195h2.47852v-2.25195zM7.7168,8.99414v2.25195h2.47656v-2.25195zM13.57617,8.99414v2.25195h2.47852v-2.25195zM10.64453,8.99805v2.25391h2.47656v-2.25391z"></path>
                    </g>
                  </g>
                </svg>
              </div>

              {/* Kubernetes Logo (provided SVG, converted to JSX) */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="100%" height="100%">
                  <g fill="none" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" style={{mixBlendMode: 'normal'}}>
                    <g transform="scale(5.33333,5.33333)">
                      <path d="M25.875,24.208c0.102,0.131 0.145,0.32 0.116,0.495l-0.276,1.208c-0.029,0.189 -0.16,0.335 -0.319,0.422l-1.118,0.524c-0.16,0.087 -0.363,0.087 -0.523,0l-1.118,-0.524c-0.16,-0.087 -0.29,-0.233 -0.319,-0.422l-0.276,-1.208c-0.029,-0.175 0.015,-0.364 0.116,-0.495l0.784,-0.975c0.101,-0.146 0.276,-0.233 0.464,-0.233h1.22c0.189,0 0.363,0.087 0.465,0.233z" fill="#534c44"></path><path d="M25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM19.338,23.111c-0.326,-0.298 -0.642,-0.596 -0.959,-0.913c-0.748,-0.74 -1.486,-1.47 -2.215,-2.229l-0.307,-0.317c-0.959,1.489 -1.515,3.266 -1.515,5.178c0,0.183 0.01,0.355 0.01,0.528l0.451,-0.154c0.988,-0.355 1.985,-0.663 2.972,-0.989c0.47,-0.154 0.94,-0.288 1.409,-0.432c0.298,-0.086 0.384,-0.461 0.154,-0.672zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM29.501,27.761c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736l-0.336,-0.019c-1.044,-0.067 -2.079,-0.163 -3.125,-0.259zM30.355,17.703c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739zM25.014,30.921c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452l-0.221,-0.365c-0.555,-0.883 -1.073,-1.796 -1.591,-2.699zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM19.185,23.783c0.297,-0.086 0.384,-0.461 0.153,-0.672c-0.326,-0.298 -0.642,-0.596 -0.959,-0.913c-0.748,-0.74 -1.486,-1.47 -2.215,-2.229l-0.307,-0.317c-0.959,1.489 -1.515,3.266 -1.515,5.178c0,0.183 0.01,0.355 0.01,0.528l0.451,-0.154c0.988,-0.355 1.985,-0.663 2.972,-0.989c0.47,-0.153 0.94,-0.287 1.41,-0.432zM44.766,28.174l-3.663,-16.12c-0.163,-0.72 -0.642,-1.316 -1.304,-1.643l-14.823,-7.166c-0.662,-0.327 -1.429,-0.327 -2.09,0l-14.823,7.166c-0.662,0.327 -1.141,0.922 -1.304,1.643l-3.663,16.12c-0.163,0.72 0.01,1.47 0.46,2.037l10.259,12.93c0.46,0.576 1.16,0.913 1.889,0.913h16.453c0.729,0 1.429,-0.336 1.889,-0.913l10.259,-12.93c0.451,-0.568 0.624,-1.317 0.461,-2.037zM39.646,28.846c-0.288,0.067 -0.575,0 -0.805,-0.144c-0.47,-0.317 -0.968,-0.49 -1.467,-0.634c-0.249,-0.067 -0.508,-0.125 -0.757,-0.163c-0.249,-0.077 -0.537,0 -0.825,0.134c-0.288,0.163 -0.547,0.135 -0.815,0.115h-0.029c-0.825,2.738 -2.637,5.053 -5.034,6.523v0.01c0.077,0.25 0.163,0.49 0.086,0.817c-0.067,0.317 -0.077,0.605 0.058,0.836c0.105,0.231 0.221,0.471 0.345,0.692c0.259,0.451 0.547,0.903 0.968,1.278c0.422,0.394 0.451,1.047 0.067,1.479c-0.393,0.423 -1.045,0.451 -1.477,0.067c-0.221,-0.202 -0.336,-0.471 -0.336,-0.749c-0.019,-0.567 -0.163,-1.066 -0.336,-1.566c-0.086,-0.24 -0.182,-0.48 -0.297,-0.72c-0.086,-0.25 -0.316,-0.423 -0.595,-0.586c-0.268,-0.144 -0.403,-0.346 -0.537,-0.567c-1.227,0.452 -2.55,0.692 -3.931,0.692c-1.361,0 -2.665,-0.24 -3.874,-0.672c-0.115,0.211 -0.249,0.403 -0.518,0.548c-0.278,0.154 -0.508,0.336 -0.594,0.586c-0.115,0.231 -0.221,0.471 -0.307,0.711c-0.173,0.5 -0.326,0.999 -0.336,1.566c-0.019,0.576 -0.508,1.028 -1.083,1.009c-0.575,-0.019 -1.026,-0.499 -1.007,-1.076c0.01,-0.298 0.144,-0.557 0.345,-0.74c0.422,-0.384 0.71,-0.826 0.978,-1.268c0.125,-0.231 0.24,-0.461 0.345,-0.701c0.134,-0.221 0.125,-0.519 0.067,-0.836c-0.067,-0.298 0,-0.538 0.077,-0.759c-2.407,-1.451 -4.248,-3.756 -5.091,-6.494c-0.221,0.019 -0.431,0.019 -0.681,-0.106c-0.297,-0.125 -0.575,-0.202 -0.825,-0.115c-0.259,0.048 -0.508,0.115 -0.757,0.192c-0.499,0.163 -0.988,0.346 -1.448,0.672c-0.47,0.336 -1.122,0.221 -1.457,-0.25c-0.326,-0.471 -0.221,-1.124 0.249,-1.451c0.24,-0.173 0.537,-0.231 0.805,-0.183c0.547,0.115 1.074,0.077 1.592,0.01c0.259,-0.029 0.518,-0.077 0.767,-0.144c0.259,-0.019 0.489,-0.211 0.7,-0.451c0.182,-0.211 0.393,-0.298 0.604,-0.375c-0.038,-0.394 -0.058,-0.797 -0.058,-1.201c0,-2.459 0.767,-4.736 2.081,-6.609c-0.173,-0.173 -0.336,-0.346 -0.403,-0.653c-0.086,-0.307 -0.201,-0.576 -0.422,-0.72c-0.201,-0.173 -0.403,-0.327 -0.614,-0.471c-0.441,-0.288 -0.892,-0.557 -1.429,-0.72c-0.556,-0.154 -0.882,-0.74 -0.719,-1.287c0.163,-0.557 0.738,-0.874 1.294,-0.72c0.278,0.086 0.508,0.279 0.633,0.519c0.259,0.5 0.614,0.893 0.988,1.258c0.182,0.183 0.384,0.355 0.585,0.509c0.182,0.192 0.47,0.25 0.786,0.269c0.336,0.01 0.546,0.154 0.767,0.298l0.01,0.01c1.803,-1.748 4.18,-2.901 6.817,-3.151c0.029,-0.259 0.048,-0.519 0.249,-0.778c0.192,-0.259 0.326,-0.519 0.297,-0.778c0.01,-0.259 0,-0.519 -0.019,-0.788c-0.058,-0.519 -0.134,-1.038 -0.355,-1.556c-0.23,-0.528 0.019,-1.143 0.547,-1.374c0.527,-0.221 1.141,0.019 1.371,0.548c0.115,0.279 0.105,0.576 0,0.826c-0.221,0.519 -0.297,1.037 -0.355,1.556c-0.019,0.269 -0.029,0.528 -0.019,0.788c-0.029,0.259 0.105,0.519 0.297,0.778c0.201,0.259 0.221,0.519 0.249,0.778c2.704,0.259 5.139,1.46 6.961,3.285c0.221,-0.144 0.431,-0.298 0.767,-0.307c0.316,-0.019 0.604,-0.077 0.786,-0.269c0.201,-0.163 0.393,-0.336 0.585,-0.519c0.364,-0.365 0.719,-0.759 0.978,-1.258c0.259,-0.519 0.892,-0.72 1.4,-0.451c0.518,0.259 0.719,0.893 0.451,1.403c-0.134,0.269 -0.364,0.452 -0.633,0.528c-0.537,0.154 -0.988,0.432 -1.419,0.72c-0.211,0.154 -0.422,0.307 -0.614,0.48c-0.221,0.144 -0.336,0.413 -0.422,0.72c-0.067,0.327 -0.249,0.509 -0.431,0.701c1.246,1.835 1.966,4.054 1.966,6.436c0,0.365 -0.019,0.73 -0.048,1.086h0.01c0.249,0.077 0.499,0.144 0.719,0.403c0.221,0.231 0.451,0.403 0.71,0.423c0.259,0.058 0.508,0.096 0.767,0.125c0.527,0.048 1.045,0.067 1.601,-0.058c0.566,-0.125 1.122,0.231 1.246,0.797c0.128,0.555 -0.227,1.112 -0.792,1.237zM31.86,20.114c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.835 -0.508,-3.545 -1.4,-4.995zM29.501,27.761c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736l-0.336,-0.019c-1.044,-0.067 -2.079,-0.163 -3.125,-0.259zM30.355,17.703c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739zM25.014,30.921c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452l-0.221,-0.365c-0.555,-0.883 -1.073,-1.796 -1.591,-2.699zM22.176,18.827c0.096,-1.038 0.192,-2.085 0.326,-3.122l0.048,-0.375c-1.85,0.259 -3.528,1.057 -4.871,2.229l0.297,0.183c0.892,0.557 1.755,1.143 2.618,1.729c0.499,0.317 0.968,0.672 1.438,1.009c0.039,-0.549 0.087,-1.106 0.144,-1.653zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM19.185,23.783c0.297,-0.086 0.384,-0.461 0.153,-0.672c-0.326,-0.298 -0.642,-0.596 -0.959,-0.913c-0.748,-0.74 -1.486,-1.47 -2.215,-2.229l-0.307,-0.317c-0.959,1.489 -1.515,3.266 -1.515,5.178c0,0.183 0.01,0.355 0.01,0.528l0.451,-0.154c0.988,-0.355 1.985,-0.663 2.972,-0.989c0.47,-0.153 0.94,-0.287 1.41,-0.432zM19.338,23.111c-0.326,-0.298 -0.642,-0.596 -0.959,-0.913c-0.748,-0.74 -1.486,-1.47 -2.215,-2.229l-0.307,-0.317c-0.959,1.489 -1.515,3.266 -1.515,5.178c0,0.183 0.01,0.355 0.01,0.528l0.451,-0.154c0.988,-0.355 1.985,-0.663 2.972,-0.989c0.47,-0.154 0.94,-0.288 1.409,-0.432c0.298,-0.086 0.384,-0.461 0.154,-0.672zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM19.338,23.111c-0.326,-0.298 -0.642,-0.596 -0.959,-0.913c-0.748,-0.74 -1.486,-1.47 -2.215,-2.229l-0.307,-0.317c-0.959,1.489 -1.515,3.266 -1.515,5.178c0,0.183 0.01,0.355 0.01,0.528l0.451,-0.154c0.988,-0.355 1.985,-0.663 2.972,-0.989c0.47,-0.154 0.94,-0.288 1.409,-0.432c0.298,-0.086 0.384,-0.461 0.154,-0.672zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM19.875,27.492c-0.451,0.058 -0.892,0.125 -1.342,0.173c-1.045,0.125 -2.071,0.25 -3.116,0.346l-0.518,0.048c0.7,1.96 2.023,3.631 3.72,4.765l0.134,-0.404c0.326,-0.999 0.671,-1.979 1.026,-2.968c0.163,-0.48 0.336,-0.951 0.518,-1.431c0.115,-0.28 -0.125,-0.577 -0.422,-0.529zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM26.606,33.62c-0.556,-0.884 -1.074,-1.796 -1.592,-2.699c-0.23,-0.394 -0.451,-0.788 -0.662,-1.191c-0.153,-0.279 -0.547,-0.279 -0.7,0c-0.221,0.413 -0.451,0.817 -0.69,1.21c-0.527,0.903 -1.055,1.806 -1.611,2.69l-0.24,0.384c0.892,0.279 1.841,0.423 2.819,0.423c1.007,0 1.985,-0.154 2.896,-0.452zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.627,28.02c-1.045,-0.067 -2.081,-0.163 -3.126,-0.259c-0.451,-0.038 -0.892,-0.086 -1.342,-0.135c-0.288,-0.038 -0.518,0.25 -0.412,0.528c0.163,0.432 0.316,0.865 0.47,1.307c0.336,0.98 0.681,1.969 0.997,2.968l0.105,0.346c1.668,-1.143 2.963,-2.795 3.643,-4.736zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM32.119,19.835l-0.259,0.279c-0.719,0.769 -1.457,1.508 -2.196,2.248c-0.47,0.48 -0.978,0.941 -1.467,1.412c0.652,0.163 1.304,0.336 1.956,0.528c1.007,0.288 2.004,0.567 3.001,0.893l0.355,0.115c0.01,-0.154 0.01,-0.317 0.01,-0.48c0,-1.834 -0.508,-3.544 -1.4,-4.995zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373zM25.312,15.33l0.048,0.375c0.134,1.037 0.23,2.085 0.326,3.122c0.038,0.413 0.077,0.817 0.105,1.23c0.019,0.317 0.374,0.48 0.633,0.298c0.326,-0.24 0.662,-0.48 1.007,-0.711c0.863,-0.586 1.716,-1.172 2.598,-1.739l0.326,-0.202c-1.371,-1.249 -3.116,-2.104 -5.043,-2.373z" fill="#534c44"></path><path d="M12.933,28.231c-0.221,0.019 -0.431,0.019 -0.681,-0.106c-0.297,-0.125 -0.575,-0.202 -0.825,-0.115c-0.259,0.048 -0.508,0.115 -0.757,0.192c-0.499,0.163 -0.988,0.346 -1.448,0.672c-0.47,0.336 -1.122,0.221 -1.457,-0.25c-0.326,-0.471 -0.221,-1.124 0.249,-1.451c0.24,-0.173 0.537,-0.231 0.805,-0.183c0.547,0.115 1.074,0.077 1.592,0.01c0.259,-0.029 0.518,-0.077 0.767,-0.144c0.259,-0.019 0.489,-0.211 0.7,-0.451c0.182,-0.211 0.393,-0.298 0.604,-0.375c0.087,0.751 0.231,1.49 0.451,2.201z" fill="none"></path><path d="M20.057,35.686c-0.115,0.211 -0.249,0.403 -0.518,0.548c-0.278,0.154 -0.508,0.336 -0.594,0.586c-0.115,0.231 -0.221,0.471 -0.307,0.711c-0.173,0.5 -0.326,0.999 -0.336,1.566c-0.019,0.576 -0.508,1.028 -1.083,1.009c-0.575,-0.019 -1.026,-0.499 -1.007,-1.076c0.01,-0.298 0.144,-0.557 0.345,-0.74c0.422,-0.384 0.71,-0.826 0.978,-1.268c0.125,-0.231 0.24,-0.461 0.345,-0.701c0.134,-0.221 0.125,-0.519 0.067,-0.836c-0.067,-0.298 0,-0.538 0.077,-0.759c0.643,0.384 1.324,0.71 2.033,0.96z" fill="none"></path><path d="M31.438,39.788c-0.393,0.423 -1.045,0.451 -1.477,0.067c-0.221,-0.202 -0.336,-0.471 -0.336,-0.749c-0.019,-0.567 -0.163,-1.066 -0.336,-1.566c-0.086,-0.24 -0.182,-0.48 -0.297,-0.72c-0.086,-0.25 -0.316,-0.423 -0.595,-0.586c-0.268,-0.144 -0.403,-0.346 -0.537,-0.567c0.719,-0.259 1.409,-0.596 2.052,-0.989v0.01c0.077,0.25 0.163,0.49 0.086,0.817c-0.067,0.317 -0.077,0.605 0.058,0.836c0.105,0.231 0.221,0.471 0.345,0.692c0.259,0.451 0.547,0.903 0.968,1.278c0.424,0.391 0.453,1.045 0.069,1.477z" fill="none"></path><path d="M39.646,28.846c-0.288,0.067 -0.575,0 -0.805,-0.144c-0.47,-0.317 -0.968,-0.49 -1.467,-0.634c-0.249,-0.067 -0.508,-0.125 -0.757,-0.163c-0.249,-0.077 -0.537,0 -0.825,0.134c-0.288,0.163 -0.547,0.135 -0.815,0.115h-0.029c0.221,-0.72 0.364,-1.47 0.441,-2.238h0.01c0.249,0.077 0.499,0.144 0.719,0.403c0.221,0.231 0.451,0.403 0.71,0.423c0.259,0.058 0.508,0.096 0.767,0.125c0.527,0.048 1.045,0.067 1.601,-0.058c0.566,-0.125 1.122,0.231 1.246,0.797c0.124,0.558 -0.231,1.115 -0.796,1.24z" fill="none"></path>
                    </g>
                  </g>
                </svg>
              </div>
            </div>

            {/* Content - top section */}
            <div className="relative z-10">
              <h2
                className="text-[2.75rem] font-bold leading-tight mb-3 max-w-[550px]"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                {card2ScrambledText || card2FinalText}
              </h2>

              <p
                className="text-base font-medium mb-8 max-w-[550px]"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-secondary)' }}
              >
                Agentic SDLC Transformation and AI-Optimized Rearchitecture
              </p>

              {/* Key Deliverables */}
              <div className="space-y-3 max-w-[600px]">
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  Claude Agents & Skills: Context-aware development acceleration
                </div>
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  Rearchitecture for Scale: Performance-optimized pipelines
                </div>
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  Monorepo + Kubernetes: Shift-left testing with local E2E stacks
                </div>
              </div>
            </div>

            {/* Content - metrics section */}
            <div className="relative z-10 mt-auto" style={{ paddingTop: '165px' }}>
              {/* Primary metric */}
              <div
                className="text-[4.5rem] font-mono font-bold leading-none"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                  marginBottom: '16px',
                } : { color: 'var(--text-accent)', marginBottom: '16px' }}
              >
                2,600+
              </div>
              <div
                className="text-xs uppercase tracking-widest mb-14"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-secondary)' }}
              >
                Developer Weeks Earned Back
              </div>

              {/* Measurement Frameworks callout */}
              <div
                className="text-sm font-medium mb-12"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                Measurement Frameworks Quantify Impact
              </div>

              <div
                className="text-sm flex items-center gap-2"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                View Case Study
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M 6 3 L 6 5 L 17.59375 5 L 3.75 18.84375 L 5.15625 20.25 L 19 6.40625 L 19 18 L 21 18 L 21 3 Z"/>
                </svg>
              </div>
            </div>
          </div>
        </Card>

        {/* Card 3: Machine Learning */}
        <Card href="/machine-learning" isOutlineMode={isOutlineMode}>
          <div ref={card3Ref} className="h-full flex flex-col relative overflow-hidden" style={{ padding: '60px' }}>
            {/* Tech Stack Logos - vertically stacked, right aligned */}
            <div
              className="absolute flex flex-col gap-6 items-end"
              style={{
                right: '80px',
                top: '50%',
                transform: 'translateY(-50%)',
                paddingRight: '16px',
              }}
            >
              {/* AWS Bedrock Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                }}
              >
                <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
                  <path d="M13.05 15.513h3.08c.214 0 .389.177.389.394v1.82a1.709 1.709 0 011.296 1.661c0 .943-.755 1.708-1.685 1.708a1.697 1.697 0 01-1.686-1.708c0-.807.554-1.484 1.297-1.662v-1.425h-2.69v4.663a.394.394 0 01-.188.338l-2.69 1.641a.386.386 0 01-.405-.002l-4.926-3.086a.397.397 0 01-.185-.336V16.3l-2.461-1.43A.395.395 0 012 14.555V9.406c0-.14.073-.27.192-.34l2.465-1.462V4.448c0-.129.062-.249.165-.322l.021-.014L9.77 1.058a.387.387 0 01.407 0l2.69 1.675c.115.072.185.2.185.336V7.6h3.856V5.683a1.71 1.71 0 01-1.296-1.662c0-.943.755-1.708 1.685-1.708.931 0 1.685.765 1.685 1.708 0 .807-.553 1.484-1.296 1.662v2.314c0 .214-.175.39-.389.391h-4.245v1.806h6.624a1.694 1.694 0 011.64-1.313c.93 0 1.685.764 1.685 1.707 0 .943-.754 1.708-1.685 1.708a1.695 1.695 0 01-1.64-1.314H13.05v1.937h4.953l.915 1.18c.255-.149.545-.227.84-.227.931 0 1.685.764 1.685 1.707 0 .943-.754 1.708-1.685 1.708-.93 0-1.685-.765-1.685-1.708 0-.346.102-.668.276-.937l-.724-.935H13.05v1.806zM9.973 1.856L7.93 3.122V6.09h-.778V3.604L5.435 4.669v2.945l2.11 1.36L9.712 7.61V5.334h.778V7.83c0 .136-.07.263-.184.335L7.963 9.638v2.081l1.422 1.009-.446.646-1.406-.998-1.53 1.005-.423-.66 1.605-1.055v-1.99L5.038 8.29l-2.26 1.34v1.676l1.972-1.189.398.677-2.37 1.429V14.3l2.166 1.258 2.27-1.368.397.677-2.176 1.311V19.3l1.876 1.175 2.365-1.426.398.678-2.017 1.216 1.918 1.201 2.298-1.403v-5.78l-4.758 2.893-.4-.675 5.158-3.136V3.289L9.972 1.856h.001zM16.13 18.47a.917.917 0 00-.908.913v.007c0 .507.406.918.908.918a.917.917 0 00.907-.913v-.013a.917.917 0 00-.907-.913v.001zm3.63-3.81a.917.917 0 00-.908.913v.007c0 .508.406.92.907.92a.917.917 0 00.908-.913v-.014a.917.917 0 00-.908-.913h.001zm1.555-4.99a.917.917 0 00-.908.913v.007c0 .507.407.918.908.918a.917.917 0 00.907-.913v-.013a.917.917 0 00-.907-.913v.001zM17.296 3.1a.917.917 0 00-.907.913v.007c0 .508.406.92.907.92a.917.917 0 00.908-.913v-.014a.917.917 0 00-.908-.913z" fill="#534c44" transform="scale(21.33334)"/>
                </svg>
              </div>

              {/* Neptune Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 80" width="100%" height="100%">
                  <path fill="#534c44" d="M65,58 C64.448,58 64,57.551 64,57 C64,56.449 64.448,56 65,56 C65.552,56 66,56.449 66,57 C66,57.551 65.552,58 65,58 L65,58 Z M55.5,66 C54.121,66 53,64.878 53,63.5 C53,62.122 54.121,61 55.5,61 C56.879,61 58,62.122 58,63.5 C58,64.878 56.879,66 55.5,66 L55.5,66 Z M51,48.5 C51,46.57 52.57,45 54.5,45 C56.43,45 58,46.57 58,48.5 C58,50.43 56.43,52 54.5,52 C52.57,52 51,50.43 51,48.5 L51,48.5 Z M45,40 C44.448,40 44,39.551 44,39 C44,38.449 44.448,38 45,38 C45.552,38 46,38.449 46,39 C46,39.551 45.552,40 45,40 L45,40 Z M42,54 C40.897,54 40,53.103 40,52 C40,50.897 40.897,50 42,50 C43.103,50 44,50.897 44,52 C44,53.103 43.103,54 42,54 L42,54 Z M63,33 C64.103,33 65,33.897 65,35 C65,36.103 64.103,37 63,37 C61.897,37 61,36.103 61,35 C61,33.897 61.897,33 63,33 L63,33 Z M65,54 C63.346,54 62,55.346 62,57 C62,57.446 62.104,57.865 62.279,58.246 L58.774,60.434 C58.117,59.732 57.244,59.245 56.257,59.077 L56.019,53.782 C58.314,53.121 60,51.005 60,48.5 C60,47.028 59.415,45.692 58.47,44.704 L61.955,38.844 C62.29,38.936 62.637,39 63,39 C65.206,39 67,37.206 67,35 C67,32.794 65.206,31 63,31 C60.794,31 59,32.794 59,35 C59,36.122 59.467,37.133 60.213,37.86 L56.841,43.531 C56.13,43.194 55.338,43 54.5,43 C53.348,43 52.278,43.358 51.394,43.966 L47.733,40.223 C47.901,39.849 48,39.437 48,39 C48,37.346 46.654,36 45,36 C43.346,36 42,37.346 42,39 C42,40.654 43.346,42 45,42 C45.486,42 45.939,41.873 46.347,41.666 L49.977,45.378 C49.362,46.266 49,47.341 49,48.5 C49,48.704 49.013,48.905 49.035,49.103 L45.371,49.865 C44.66,48.748 43.42,48 42,48 C39.794,48 38,49.794 38,52 C38,54.206 39.794,56 42,56 C44.206,56 46,54.206 46,52 C46,51.925 45.982,51.855 45.979,51.782 L49.619,51.024 C50.467,52.656 52.105,53.811 54.025,53.976 L54.26,59.195 C52.384,59.738 51,61.452 51,63.5 C51,65.981 53.019,68 55.5,68 C57.981,68 60,65.981 60,63.5 C60,63.033 59.908,62.59 59.775,62.166 L63.722,59.703 C64.11,59.888 64.541,60 65,60 C66.654,60 68,58.654 68,57 C68,55.346 66.654,54 65,54 L65,54 Z M31.5,15 C41.224,15 48,18.162 48,21 C48,23.838 41.224,27 31.5,27 C21.776,27 15,23.838 15,21 C15,18.162 21.776,15 31.5,15 L31.5,15 Z M48,60.02 C48,62.772 41.224,65.84 31.5,65.84 C21.776,65.84 15,62.772 15,60.02 L15,51.78 C17.957,54.302 23.937,55.954 31.167,55.954 C32.87,55.954 34.591,55.859 36.279,55.672 L36.059,53.684 C34.443,53.863 32.797,53.954 31.167,53.954 C21.496,53.954 15,50.919 15,48.086 L15,38.287 C17.957,40.809 23.937,42.46 31.167,42.46 C34.632,42.46 37.996,42.076 40.895,41.348 L40.408,39.409 C37.666,40.097 34.471,40.46 31.167,40.46 C21.496,40.46 15,37.426 15,34.592 L15,24.722 C17.997,27.305 24.104,29 31.5,29 C38.896,29 45.003,27.305 48,24.722 L48,36 L50,36 L50,21 C50,16.439 42.047,13 31.5,13 C20.953,13 13,16.439 13,21 L13,60.02 C13,64.478 20.953,67.84 31.5,67.84 C42.047,67.84 50,64.478 50,60.02 L50,55 L48,55 L48,60.02 Z"/>
                </svg>
              </div>

              {/* LangChain Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <svg width="100%" height="100%" viewBox="0 0 80 41" xmlns="http://www.w3.org/2000/svg">
                  <path d="M61.5139 11.1569C60.4527 11.1569 59.4549 11.568 58.708 12.3148L55.6899 15.3248C54.8757 16.1368 54.4574 17.2643 54.5431 18.4202C54.5492 18.4833 54.5553 18.5464 54.5615 18.6115C54.6696 19.4988 55.0594 20.2986 55.6899 20.9254C56.1246 21.3589 56.6041 21.6337 57.1857 21.825C57.2163 22 57.2326 22.177 57.2326 22.3541C57.2326 23.1519 56.9225 23.9008 56.3592 24.4625L56.1735 24.6477C55.1655 24.3037 54.3247 23.8011 53.5656 23.044C52.5576 22.0386 51.8903 20.7687 51.6393 19.3747L51.6046 19.1813L51.4515 19.3055C51.3475 19.3889 51.2495 19.4785 51.1577 19.57L48.1396 22.58C46.5928 24.1226 46.5928 26.636 48.1396 28.1786C48.913 28.9499 49.9292 29.3366 50.9475 29.3366C51.9658 29.3366 52.98 28.9499 53.7534 28.1786L56.7715 25.1687C58.3183 23.626 58.3183 21.1147 56.7715 19.57C56.3592 19.159 55.8675 18.8496 55.3104 18.6502C55.2798 18.469 55.2634 18.2879 55.2634 18.1109C55.2634 17.2439 55.6063 16.4217 56.2348 15.7949C57.2449 16.1388 58.1407 16.6965 58.8978 17.4515C59.9038 18.4548 60.5691 19.7227 60.8241 21.1208L60.8588 21.3141L61.0119 21.19C61.116 21.1066 61.2139 21.017 61.3078 20.9234L64.3259 17.9135C65.8727 16.3708 65.8747 13.8575 64.3259 12.3148C63.577 11.568 62.5811 11.1569 61.518 11.1569H61.5139Z" fill="#534c44"/>
                  <path d="M59.8966 0.148865H20.4063C9.15426 0.148865 0 9.27841 0 20.5001C0 31.7217 9.15426 40.8513 20.4063 40.8513H59.8966C71.1486 40.8513 80.3029 31.7217 80.3029 20.5001C80.3029 9.27841 71.1486 0.148865 59.8966 0.148865ZM40.4188 32.0555C39.7678 32.1898 39.0352 32.2142 38.5373 31.6953C38.3536 32.1165 37.9251 31.8947 37.5945 31.8398C37.5639 31.9252 37.5374 32.0005 37.5088 32.086C36.4089 32.1593 35.5845 31.04 35.0601 30.1954C34.0193 29.6337 32.8378 29.2918 31.7746 28.7036C31.7134 29.6724 31.9257 30.8731 31.0012 31.4979C30.9543 33.36 33.8255 31.7177 34.0887 33.1056C33.8847 33.128 33.6582 33.073 33.4949 33.2297C32.746 33.9563 31.8869 32.6803 31.0237 33.2074C29.8646 33.7894 29.7483 34.2656 28.3137 34.3857C28.2342 34.2656 28.2668 34.1862 28.3341 34.113C28.7382 33.6449 28.7668 33.0934 29.4565 32.8939C28.7464 32.782 28.1525 33.1728 27.5546 33.4821C26.7771 33.7996 26.7833 32.7657 25.5875 33.537C25.4548 33.4292 25.5181 33.3315 25.5936 33.2481C25.8976 32.8777 26.2976 32.8227 26.7486 32.8431C24.5304 31.6098 23.4856 34.3511 22.4612 32.9876C22.1531 33.069 22.0368 33.3457 21.8429 33.5411C21.6756 33.358 21.8021 33.1361 21.8103 32.9204C21.6103 32.8268 21.3572 32.782 21.4164 32.4625C21.0246 32.3302 20.7512 32.5622 20.4594 32.782C20.1961 32.5785 20.6369 32.2814 20.7185 32.0697C20.9532 31.6627 21.4878 31.9863 21.7592 31.6932C22.5306 31.2557 23.606 31.9659 24.4876 31.8459C25.1671 31.9313 26.0078 31.2353 25.667 30.5413C24.9406 29.6154 25.0691 28.4045 25.0528 27.2974C24.963 26.6522 23.4101 25.83 22.9612 25.134C22.4061 24.5072 21.9735 23.7807 21.5409 23.0664C19.9798 20.0523 20.4716 16.1795 18.5044 13.3812C17.6147 13.8717 16.4556 13.6397 15.6884 12.9823C15.2741 13.3588 15.2557 13.8513 15.2231 14.3744C14.2293 13.3833 14.3538 11.5109 15.1476 10.4079C15.4721 9.97239 15.8598 9.61421 16.2924 9.29876C16.3903 9.22754 16.423 9.15834 16.4209 9.04844C17.2066 5.52362 22.5653 6.20335 24.259 8.70044C25.4875 10.237 25.8589 12.27 27.2526 13.6967C29.1279 15.744 31.2645 17.5471 32.9949 19.7267C34.6315 21.7191 35.8008 24.0554 36.8211 26.4101C37.2374 27.1915 37.2415 28.1501 37.8578 28.8176C38.1618 29.2206 39.6474 30.3175 39.325 30.7062C39.5107 31.1091 40.8983 31.6647 40.4167 32.0555H40.4188ZM66.4449 20.032L63.4269 23.0419C62.6228 23.8438 61.6291 24.4421 60.5516 24.7697L60.4965 24.786L60.4761 24.8389C60.1251 25.7629 59.5925 26.5871 58.8905 27.2852L55.8724 30.2951C54.5562 31.6078 52.8054 32.3302 50.9402 32.3302C49.0751 32.3302 47.3242 31.6078 46.008 30.2951C43.2879 27.5823 43.2879 23.1701 46.008 20.4573L49.0261 17.4474C49.8342 16.6415 50.8015 16.0615 51.8973 15.7257L51.9524 15.7094L51.9728 15.6565C52.3238 14.7325 52.8584 13.9063 53.5625 13.2021L56.5805 10.1922C57.8967 8.87953 59.6476 8.15706 61.5127 8.15706C63.3779 8.15706 65.1287 8.87953 66.4449 10.1922C67.7612 11.5048 68.4856 13.251 68.4856 15.1111C68.4856 16.9712 67.7612 18.7193 66.4449 20.03V20.032Z" fill="#534c44"/>
                  <path d="M28.1422 28.4126C27.8769 29.4424 27.7912 31.1946 26.4485 31.2455C26.3383 31.8398 26.8607 32.0636 27.3382 31.8723C27.8096 31.6566 28.034 32.0433 28.1932 32.4279C28.9217 32.5337 29.9992 32.1857 30.04 31.3249C28.9523 30.7001 28.6156 29.5116 28.1442 28.4106L28.1422 28.4126Z" fill="#534c44"/>
                </svg>
              </div>

              {/* Hugging Face Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0,0,256,256">
                  <g fill="#534c44" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" strokeDasharray="" strokeDashoffset="0" style={{mixBlendMode: 'normal'}}>
                    <g transform="scale(5.12,5.12)">
                      <path d="M25,4c-11.028,0 -20,8.972 -20,20c0,2.17 0.34262,4.28073 1.01563,6.30273c-0.164,-0.035 -0.32289,-0.06398 -0.46289,-0.08398c-0.133,-0.018 -0.26363,-0.02734 -0.39062,-0.02734c-0.991,0 -1.88086,0.51495 -2.38086,1.37695c-0.507,0.872 -0.50691,1.9617 -0.00391,2.8457c0.037,0.064 0.07523,0.12745 0.11523,0.18945c-0.287,0.219 -0.53266,0.49912 -0.72266,0.82813c-0.584,1.012 -0.49422,2.21053 0.17578,3.14453c-0.356,0.48 -0.52928,1.06116 -0.48828,1.66016c0.05,0.717 0.41314,1.41034 0.99414,1.90234c0.496,0.419 1.03417,0.85559 1.70117,1.18359c3.405,1.673 7.00522,2.55233 10.69922,2.61133h0.08984l0.08984,-0.00586c0.216,-0.016 0.43244,-0.02997 0.64844,-0.04297c0.507,-0.032 1.03131,-0.06691 1.57031,-0.12891c1.212,-0.141 2.71039,-0.65331 3.90039,-2.07031c1.133,0.197 2.28822,0.31445 3.44922,0.31445c1.099,0 2.2013,-0.09348 3.2793,-0.27148c1.183,1.381 2.66233,1.88834 3.86133,2.02734c0.539,0.062 1.06331,0.09495 1.57031,0.12695c0.216,0.014 0.43244,0.02697 0.64844,0.04297l0.08984,0.00781l0.08984,-0.00195c3.694,-0.059 7.29422,-0.93637 10.69922,-2.60937c0.667,-0.328 1.20617,-0.76459 1.70117,-1.18359c0.582,-0.492 0.94414,-1.1873 0.99414,-1.9043c0.041,-0.599 -0.13128,-1.18016 -0.48828,-1.66016c0.67,-0.934 0.76078,-2.13253 0.17578,-3.14453c-0.19,-0.329 -0.43566,-0.60717 -0.72266,-0.82617c0.04,-0.062 0.07923,-0.12741 0.11523,-0.19141c0.503,-0.884 0.50114,-1.9737 -0.00586,-2.8457c-0.501,-0.861 -1.38986,-1.375 -2.38086,-1.375c-0.127,0 -0.25762,0.00934 -0.39062,0.02734c-0.074,0.01 -0.15923,0.02506 -0.24023,0.03906c0.664,-2.013 1.00391,-4.11781 1.00391,-6.25781c0,-11.028 -8.972,-20 -20,-20zM25,6c9.925,0 18,8.075 18,18c0,1.384 -0.16366,2.74808 -0.47266,4.08008c-0.471,-0.443 -1.09049,-0.70312 -1.77148,-0.70312c-0.365,0 -0.72908,0.07466 -1.08008,0.22266c-0.648,0.273 -1.58055,0.7338 -2.18555,1.5918c-0.11,0.156 -0.21912,0.31175 -0.32812,0.46875c-0.717,-1.166 -1.79439,-1.37325 -2.27539,-1.40625c-0.068,-0.005 -0.13417,-0.00781 -0.20117,-0.00781c-1.12,0 -2.11611,0.65119 -2.66211,1.74219c-0.197,0.394 -0.3795,0.79431 -0.5625,1.19531c-0.208,0.454 -0.40323,0.88111 -0.61523,1.28711c-0.581,1.111 -1.32122,2.1117 -2.19922,2.9707c-0.049,0.048 -0.09558,0.09649 -0.14258,0.14648c-0.656,0.713 -2.65195,2.88236 -1.50195,5.94336c0.044,0.117 0.09948,0.21027 0.14648,0.32227c-1.492,0.178 -3.01009,0.17151 -4.49609,-0.02148c0.044,-0.104 0.09572,-0.19078 0.13672,-0.30078c1.15,-3.061 -0.84595,-5.23036 -1.50195,-5.94336c-0.046,-0.05 -0.09358,-0.09949 -0.14258,-0.14648c-0.878,-0.86 -1.61822,-1.85775 -2.19922,-2.96875c-0.212,-0.406 -0.40823,-0.83506 -0.61523,-1.28906c-0.183,-0.401 -0.36745,-0.80131 -0.56445,-1.19531c-0.546,-1.091 -1.54016,-1.74219 -2.66016,-1.74219c-0.066,0 -0.13412,0.00281 -0.20312,0.00781c-0.481,0.033 -1.55939,0.2432 -2.27539,1.4082c-0.109,-0.157 -0.21813,-0.31275 -0.32812,-0.46875c-0.605,-0.859 -1.53755,-1.32175 -2.18555,-1.59375c-0.351,-0.148 -0.71508,-0.22266 -1.08008,-0.22266c-0.604,0 -1.15166,0.21436 -1.59766,0.56836c-0.287,-1.284 -0.43555,-2.60131 -0.43555,-3.94531c0,-9.925 8.075,-18 18,-18zM18.29688,15.28125c-0.83344,0.04702 -1.62277,0.49911 -2.10352,1.25586c-0.604,0.951 -0.49433,2.24242 0.26367,3.10742c0.733,0.837 1.07266,0.82455 1.72266,-0.06445c0.519,-0.708 0.71281,-0.76784 1.50781,-0.46484c0.951,0.363 1.19652,0.21903 1.35352,-0.79297c0.02,-0.131 0.01948,-0.26378 0.02148,-0.30078c0.003,-1.297 -0.77173,-2.32144 -1.92773,-2.64844c-0.27825,-0.0785 -0.56008,-0.10747 -0.83789,-0.0918zM31.50391,15.28711c-0.63621,0.03131 -1.25656,0.29281 -1.72656,0.75781c-0.734,0.727 -0.96742,1.86355 -0.60742,2.81055c0.161,0.425 0.40475,0.5522 0.84375,0.4082c1.29,-0.425 1.29142,-0.41547 2.10742,0.64453c0.02,0.026 0.04055,0.05308 0.06055,0.08008c0.243,0.32 0.52038,0.36972 0.85938,0.13672c0.781,-0.537 1.20103,-1.26419 1.20703,-1.74219c-0.002,-1.564 -0.60937,-2.47944 -1.60937,-2.89844c-0.3645,-0.15225 -0.75304,-0.21605 -1.13477,-0.19727zM12.5,20c-0.82843,0 -1.5,0.67157 -1.5,1.5c0,0.82843 0.67157,1.5 1.5,1.5c0.82843,0 1.5,-0.67157 1.5,-1.5c0,-0.82843 -0.67157,-1.5 -1.5,-1.5zM37.5,20c-0.82843,0 -1.5,0.67157 -1.5,1.5c0,0.82843 0.67157,1.5 1.5,1.5c0.82843,0 1.5,-0.67157 1.5,-1.5c0,-0.82843 -0.67157,-1.5 -1.5,-1.5zM18.72656,23.89648c-0.39881,-0.02152 -0.57748,0.2568 -0.55273,0.8418c0.153,3.68 2.66666,6.90081 6.84766,6.88281c4.184,0.014 6.69317,-3.2357 6.82617,-6.8457c0.031,-0.847 -0.28112,-1.06023 -1.07812,-0.74023c-0.6,0.24 -1.18481,0.52227 -1.75781,0.82227c-2.524,1.321 -5.06453,1.46611 -7.64453,0.16211c-0.711,-0.359 -1.42997,-0.70595 -2.16797,-1.00195c-0.18225,-0.07325 -0.33972,-0.11392 -0.47266,-0.12109zM40.75586,29.37695c0.357,0 0.61597,0.29633 0.66797,0.73633c0.063,0.531 -0.01228,1.04098 -0.36328,1.45898c-0.576,0.687 -1.17244,1.35939 -1.77344,2.02539c-0.657,0.728 -2.37305,2.4628 -2.37305,2.7168c0,0.32 0.25813,0.58008 0.57813,0.58008c0.273,0 1.13573,-0.89328 1.42773,-1.11328c1.366,-1.026 2.71133,-2.07764 4.11133,-3.05664c0.416,-0.291 0.96947,-0.45439 1.48047,-0.52539c0.039,-0.005 0.07823,-0.00781 0.11523,-0.00781c0.599,0 0.96344,0.67842 0.64844,1.23242c-0.144,0.253 -0.35584,0.49206 -0.58984,0.66406c-0.561,0.412 -1.14633,0.79306 -1.73633,1.16406c-0.762,0.481 -4.04972,2.58842 -4.13672,2.73242c-0.009,0.007 -0.02344,0.01563 -0.02344,0.01563c0,0 0.00581,0.00777 0.00781,0.00977c-0.043,0.081 -0.07422,0.16958 -0.07422,0.26758c0,0.32 0.26008,0.57813 0.58008,0.57813c0.105,0 0.1982,-0.03498 0.2832,-0.08398c0.006,-0.001 0.01158,-0.00091 0.01758,-0.00391c0.891,-0.48 1.74553,-1.02919 2.64453,-1.49219c0.823,-0.424 1.67311,-0.80372 2.53711,-1.13672c0.145,-0.056 0.30013,-0.09375 0.45313,-0.09375c0.248,0 0.48734,0.09976 0.65234,0.38477c0.227,0.393 0.12664,0.88123 -0.31836,1.24023c-0.345,0.278 -0.72977,0.53108 -1.13477,0.70508c-1.005,0.433 -2.04183,0.79556 -3.04883,1.22656c-0.374,0.16 -0.7373,0.39383 -1.0293,0.67383c-0.13,0.125 -0.15341,0.48806 -0.06641,0.66406c0.066,0.133 0.36827,0.20117 0.57227,0.20117h0.01367c0.203,-0.003 0.41252,-0.11494 0.60352,-0.21094c0.623,-0.313 1.22733,-0.66708 1.86133,-0.95508c0.539,-0.244 1.09883,-0.4757 1.67383,-0.5957c0.025,-0.005 0.05013,-0.00781 0.07813,-0.00781c0.233,0 0.54899,0.17352 0.70898,0.35352c0.258,0.288 0.1078,0.66172 -0.1582,0.88672c-0.402,0.34 -0.82397,0.68502 -1.29297,0.91602c-3.102,1.524 -6.37666,2.35125 -9.84766,2.40625c-0.712,-0.053 -1.42872,-0.08106 -2.13672,-0.16406c-1.805,-0.21 -2.88705,-1.31441 -3.49805,-2.94141c-0.583,-1.553 0.08151,-2.77672 1.10352,-3.88672c0.023,-0.025 0.04631,-0.04731 0.07031,-0.07031c1.043,-1.02 1.89631,-2.18366 2.57031,-3.47266c0.429,-0.822 0.78031,-1.68367 1.19531,-2.51367c0.175,-0.35 0.45405,-0.63867 0.87305,-0.63867c0.021,0 0.04345,0.00095 0.06445,0.00195c0.477,0.033 0.71814,0.39192 0.86914,0.79492c0.409,1.094 0.13581,2.06325 -0.61719,2.90625c-0.308,0.345 -0.73786,0.58903 -1.00586,0.95703c-0.178,0.245 -0.30098,0.67645 -0.20898,0.93945c0.105,0.299 0.39203,0.40625 0.70703,0.40625c0.091,0 0.18534,-0.00939 0.27734,-0.02539c0.468,-0.081 0.73252,-0.39439 0.97852,-0.77539c1.106,-1.715 2.199,-3.43847 3.375,-5.10547c0.291,-0.413 0.84013,-0.6983 1.32813,-0.9043c0.107,-0.045 0.20969,-0.06445 0.30469,-0.06445zM9.0332,29.37891c0.095,0 0.19769,0.01945 0.30469,0.06445c0.488,0.205 1.03517,0.4913 1.32617,0.9043c1.176,1.667 2.27095,3.39047 3.37695,5.10547c0.246,0.381 0.50956,0.69339 0.97656,0.77539c0.092,0.016 0.18634,0.02539 0.27734,0.02539c0.315,0 0.60203,-0.10825 0.70703,-0.40625c0.093,-0.263 -0.02903,-0.69445 -0.20703,-0.93945c-0.267,-0.368 -0.69786,-0.61203 -1.00586,-0.95703c-0.753,-0.843 -1.02619,-1.81125 -0.61719,-2.90625c0.151,-0.403 0.39214,-0.75997 0.86914,-0.79297c0.022,-0.001 0.04245,-0.00391 0.06445,-0.00391c0.418,0 0.69805,0.28867 0.87305,0.63867c0.415,0.83 0.76631,1.69267 1.19531,2.51367c0.674,1.289 1.52731,2.45166 2.57031,3.47266c0.024,0.023 0.04731,0.04631 0.07031,0.07031c1.021,1.111 1.68652,2.33372 1.10352,3.88672c-0.61,1.625 -1.69305,2.72945 -3.49805,2.93945c-0.709,0.082 -1.42472,0.11106 -2.13672,0.16406c-3.471,-0.055 -6.74566,-0.88225 -9.84766,-2.40625c-0.469,-0.23 -0.89097,-0.57502 -1.29297,-0.91602c-0.266,-0.225 -0.4162,-0.59872 -0.1582,-0.88672c0.161,-0.179 0.47598,-0.35352 0.70898,-0.35352c0.028,0 0.05312,0.00181 0.07813,0.00781c0.575,0.12 1.13383,0.3507 1.67383,0.5957c0.634,0.287 1.23933,0.64108 1.86133,0.95508c0.191,0.096 0.40052,0.20794 0.60352,0.21094h0.01563c0.204,0 0.50627,-0.06817 0.57227,-0.20117c0.087,-0.176 0.06259,-0.54006 -0.06641,-0.66406c-0.292,-0.28 -0.6553,-0.51283 -1.0293,-0.67383c-1.006,-0.431 -2.04283,-0.79161 -3.04883,-1.22461c-0.406,-0.175 -0.78977,-0.42903 -1.13477,-0.70703c-0.445,-0.359 -0.54536,-0.84723 -0.31836,-1.24023c0.165,-0.285 0.40434,-0.38281 0.65234,-0.38281c0.153,0 0.30812,0.03775 0.45313,0.09375c0.864,0.332 1.71411,0.71272 2.53711,1.13672c0.899,0.464 1.75353,1.01123 2.64453,1.49023c0.005,0.003 0.01158,0.00291 0.01758,0.00391c0.085,0.049 0.1782,0.08594 0.2832,0.08594c0.32,0 0.58008,-0.26008 0.58008,-0.58008c0,-0.099 -0.03122,-0.18658 -0.07422,-0.26758c0.001,-0.003 0.00586,-0.00977 0.00586,-0.00977c0,0 -0.01344,-0.00962 -0.02344,-0.01562c-0.087,-0.144 -3.37372,-2.25142 -4.13672,-2.73242c-0.59,-0.372 -1.17533,-0.75206 -1.73633,-1.16406c-0.234,-0.172 -0.44584,-0.41106 -0.58984,-0.66406c-0.316,-0.554 0.04944,-1.23047 0.64844,-1.23047c0.037,0 0.07623,0.00281 0.11523,0.00781c0.511,0.071 1.06447,0.23244 1.48047,0.52344c1.4,0.978 2.74533,2.03159 4.11133,3.05859c0.292,0.22 1.15473,1.11133 1.42773,1.11133c0.32,0 0.57813,-0.25812 0.57813,-0.57812c0,-0.254 -1.71605,-1.99075 -2.37305,-2.71875c-0.601,-0.666 -1.19744,-1.33644 -1.77344,-2.02344c-0.067,-0.08 -0.11506,-0.16595 -0.16406,-0.25195c-0.062,-0.139 -0.10997,-0.28187 -0.16797,-0.42187c-0.058,-0.25 -0.06425,-0.51416 -0.03125,-0.78516c0.052,-0.44 0.31097,-0.73633 0.66797,-0.73633z"></path>
                    </g>
                  </g>
                </svg>
              </div>
            </div>

            {/* Content - top section */}
            <div className="relative z-10">
              <h2
                className="text-[2.75rem] font-bold leading-tight mb-3 max-w-[550px]"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                {card3ScrambledText || card3FinalText}
              </h2>

              <p
                className="text-base font-medium mb-8 max-w-[550px]"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-secondary)' }}
              >
                Production ML Systems & GenAI Service Portfolio
              </p>

              {/* Key Deliverables */}
              <div className="space-y-3 max-w-[600px]">
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  Vector Search & RAG: Multimodal embedding pipeline for semantic retrieval
                </div>
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  GenAI Applications: Content evaluation, Bedrock search, and natural language SQL
                </div>
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  Recommendation Engines: Neptune graphs, ARM, and Amazon Personalize
                </div>
              </div>
            </div>

            {/* Content - metrics section */}
            <div className="relative z-10 mt-auto" style={{ paddingTop: '165px' }}>
              {/* Primary metric */}
              <div
                className="text-[4.5rem] font-mono font-bold leading-none"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                  marginBottom: '16px',
                } : { color: 'var(--text-accent)', marginBottom: '16px' }}
              >
                6
              </div>
              <div
                className="text-xs uppercase tracking-widest mb-14"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-secondary)' }}
              >
                Production ML Services Launched
              </div>

              {/* Callout */}
              <div
                className="text-sm mb-12"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                Scalable ML Infrastructure & Applications
              </div>

              <div
                className="text-sm flex items-center gap-2"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                View Case Study
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M 6 3 L 6 5 L 17.59375 5 L 3.75 18.84375 L 5.15625 20.25 L 19 6.40625 L 19 18 L 21 18 L 21 3 Z"/>
                </svg>
              </div>
            </div>
          </div>
        </Card>

        {/* Card 4: Data Engineering */}
        <Card href="/data-engineering" isOutlineMode={isOutlineMode}>
          <div ref={card4Ref} className="h-full flex flex-col relative overflow-hidden" style={{ padding: '60px' }}>
            {/* Tech Stack Logos - vertically stacked, right aligned */}
            <div
              className="absolute flex flex-col gap-6 items-end"
              style={{
                right: '80px',
                top: '50%',
                transform: 'translateY(-50%)',
                paddingRight: '16px',
              }}
            >
              {/* AWS Lake Formation Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96 96" width="100%" height="100%">
                  <path fill="#534c44" d="M64.376 54.1228c.8965 1.0458 2.2278 1.7097 3.7094 1.7097 2.6931 0 4.8832-2.1914 4.8832-4.8845h-2.3499c0 1.3982-1.1362 2.5345-2.5333 2.5345-1.397 0-2.5344-1.1363-2.5344-2.5345h-2.3499c0 1.3982-1.1362 2.5345-2.5333 2.5345-1.3982 0-2.5344-1.1363-2.5344-2.5345h-2.35c0 1.3982-1.1373 2.5345-2.5344 2.5345-1.397 0-2.5344-1.1363-2.5344-2.5345h-2.35c0 1.3982-1.1373 2.5345-2.5332 2.5345v2.35c1.4828 0 2.8117-.6639 3.7082-1.7097.8965 1.0458 2.2278 1.7097 3.7094 1.7097 1.4828 0 2.8129-.6639 3.7094-1.7097.8953 1.0458 2.2266 1.7097 3.7094 1.7097 1.4817 0 2.8117-.6639 3.7082-1.7097Zm-1.0316-15.987h-2.3499c0 1.3971-1.1374 2.5333-2.5345 2.5333-1.397 0-2.5332-1.1362-2.5332-2.5333h-2.35c0 1.3971-1.1373 2.5333-2.5356 2.5333-1.3958 0-2.5332-1.1362-2.5332-2.5333h-2.35c0 1.3971-1.1373 2.5333-2.5332 2.5333v2.35c1.4828 0 2.8117-.6639 3.7082-1.7096.8965 1.0457 2.2266 1.7096 3.7082 1.7096 1.4829 0 2.8141-.6639 3.7106-1.7096.8953 1.0457 2.2254 1.7096 3.7082 1.7096 1.4828 0 2.8129-.6639 3.7094-1.7096.8965 1.0457 2.2266 1.7096 3.7082 1.7096v-2.35c-1.3958 0-2.5332-1.1362-2.5332-2.5333ZM44.7163 70.2344c1.4817 0 2.8118-.6638 3.7083-1.7096.8965 1.0458 2.2265 1.7096 3.7094 1.7096 1.4828 0 2.8129-.6638 3.7082-1.7096.8965 1.0458 2.2266 1.7096 3.7094 1.7096v-2.35c-1.3971 0-2.5344-1.1362-2.5344-2.5333h-2.35c0 1.3971-1.1362 2.5333-2.5332 2.5333-1.3971 0-2.5345-1.1362-2.5345-2.5333h-2.3499c0 1.3971-1.1374 2.5333-2.5333 2.5333-1.3982 0-2.5356-1.1362-2.5356-2.5333h-2.3499c0 1.3971-1.1362 2.5333-2.5333 2.5333v2.35c1.4829 0 2.8129-.6638 3.7083-1.7096.8965 1.0458 2.2277 1.7096 3.7105 1.7096ZM28.0258 24.4999h1.175v-1.175h-1.175v1.175Zm-2.3511 1.175v-3.525c0-.6498.5264-1.175 1.1761-1.175h3.525c.6485 0 1.1749.5252 1.1749 1.175v3.525c0 .6486-.5264 1.175-1.1749 1.175h-3.525c-.6497 0-1.1761-.5264-1.1761-1.175Zm9.401-5.875h1.1749v-1.175h-1.1749v1.175Zm-2.35 1.175v-3.525c0-.6498.5252-1.175 1.175-1.175h3.5249c.6486 0 1.175.5252 1.175 1.175v3.525c0 .6486-.5264 1.175-1.175 1.175h-3.5249c-.6498 0-1.175-.5264-1.175-1.175Zm3.5249 7.05h1.175v-1.175h-1.175v1.175Zm2.35-3.525c.6486 0 1.175.5252 1.175 1.175v3.525c0 .6486-.5264 1.175-1.175 1.175h-3.5249c-.6498 0-1.175-.5264-1.175-1.175v-3.525c0-.6498.5252-1.175 1.175-1.175h3.5249Zm-4.4802 9.4881c-1.1926 0-5.6293-.0458-8.9792-.6603l7.4012 11.6936c.1199.188.1833.4054.1833.6286v1.4817c.8143.195 1.5839.2009 2.35.0153v-1.497c0-.2221.0622-.4394.1809-.6286l7.7102-11.6736c-2.5861.4582-5.9936.6403-8.8464.6403Zm-13.0681-2.9469c-.0082-.0552-.0188-.1598-.0188-.215 0-.7226 0-2.921 11.6687-3.149l.0458 2.35c-3.7575.0729-6.5352.4312-8.1179.799 1.7284.403 4.9349.8119 9.4903.8119 4.3885 0 7.6667-.3948 9.4691-.8131-.6016-.1421-1.4358-.2972-2.5849-.4371l.2855-2.3323c4.2017.5123 5.916 1.316 5.916 2.7706 0 .3102-.1116.7356-.5099 1.1421l-9.2706 14.0213v2.0104c0 .4794-.2926.9106-.7367 1.0892-.9036.3631-1.8106.5441-2.7365.5441-.9141 0-1.8459-.1775-2.8082-.5311-.4618-.1704-.7684-.6099-.7684-1.1022v-2.0092l-9.0192-14.2504c-.0023-.0036-.0047-.0071-.007-.0106l-.0094-.0165-.1751-.2773.0177-.0117c-.0635-.1199-.1105-.2479-.1305-.3831Zm11.6734 22.8338v-3.525h-2.3499v2.35h-2.35c-.4559 0-.8718.2632-1.0645.6756-.1939.4136-.1304.9013.161 1.2514l5.8748 7.05c.2233.2679.5546.423.9036.423.3478 0 .6791-.1551.9024-.423l5.8748-7.05c.2914-.3501.3549-.8378.161-1.2514-.1927-.4124-.6086-.6756-1.0633-.6756h-2.35v-2.35h-2.3499v3.525c0 .6486.5252 1.175 1.1749 1.175h1.0164l-3.3663 4.0397-3.3675-4.0397h1.0175c.6486 0 1.175-.5264 1.175-1.175Zm43.4741-2.35c0 15.5488-12.651 28.2-28.2006 28.2-15.5484 0-28.1994-12.6512-28.1994-28.2 0-4.2981.9423-8.4236 2.7988-12.2623l2.1161 1.0234c-1.7013 3.5168-2.5649 7.298-2.5649 11.2389 0 14.2528 11.597 25.85 25.8494 25.85 14.2536 0 25.8507-11.5972 25.8507-25.85 0-14.2539-11.5971-25.85-25.8507-25.85-1.511 0-3.0232.1304-4.4966.3889l-.4054-2.3147c1.6062-.282 3.2559-.4242 4.902-.4242 15.5496 0 28.2006 12.6501 28.2006 28.2Z"/>
                </svg>
              </div>

              {/* Apache Airflow Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '18px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <svg fill="#534c44" viewBox="0 0 24 24" role="img" xmlns="http://www.w3.org/2000/svg" stroke="#534c44" strokeWidth="0.168" width="100%" height="100%">
                  <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                  <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <title>Apache Airflow icon</title>
                    <path d="M12.033 10.713c-.691 0-1.263.564-1.263 1.255 0 .69.572 1.262 1.263 1.262a1.258 1.258 0 000-2.517zm0 .797c.258 0 .457.2.457.458s-.2.458-.457.458c-.259 0-.458-.2-.458-.458s.199-.458.458-.458zM.65.001C.577.001.544.013.3.041.053.07-.027.364.03.673c.055.31.072.205.196.384.124.18 4.228 4.144 6.352 6.216l-3.625 3.72-.133.133-.007.008c-1.25 1.282-1.91 3.251-2.296 5.433C.13 18.748.03 21.161 0 23.35a.63.63 0 00.554.62.757.757 0 00.118 0 .608.608 0 00.384-.184h.007l6.209-6.37c4.021 3.721 4.12 5.08 9.294 6.067 2.182.386 4.595.487 6.784.517a.626.626 0 00.62-.546l.008-.008a.746.746 0 00-.008-.133.621.621 0 00-.177-.37v-.008l-6.378-6.208L21.047 13c.046-.043.089-.088.133-.133l.007-.008c1.25-1.281 1.918-3.244 2.304-5.426.385-2.182.479-4.595.509-6.783a.627.627 0 00-.546-.62L23.446.02a.668.668 0 00-.133.008.608.608 0 00-.369.178h-.007l-6.209 6.37-3.72-3.624-.133-.133-.008-.008C11.586 1.562 9.617.894 7.434.509 5.18.26 2.917.034.65 0zm.48.835c2.039.038 4.213.12 6.164.465 2.105.372 3.927 1.03 5.012 2.089h.008l.103.11c.023.023.049.053.074.082.002.003.005.004.007.008l.008.008.015.015c0 .002-.002.008 0 .008.002.003.005.004.007.008.128.176.354.52.42.664.049.101.093.208.134.325.179.519.268 1.166.206 1.985v.008c-.014.181-.038.37-.066.569-.014.095-.033.2-.052.31a10.753 10.753 0 01-.14.642c-.028.115-.056.232-.089.347v.008c-.13.468-.3.976-.51 1.52-.052.138-.103.272-.161.414-.046.111-.099.225-.148.34l-.258.051c-.875-1.655-1.831-2.8-2.688-3.455-.432-.33-.837-.54-1.225-.627h-.007c-.387-.084-.8-.027-1.07.251l.038.075.32-.239zm22.036.288c-.038 2.04-.12 4.218-.465 6.17-.372 2.105-1.031 3.928-2.09 5.013l-.11.103c-.03.026-.062.059-.096.088l-.015.016h-.007c-.174.126-.527.36-.672.428-.101.047-.21.1-.325.14h-.007c-.516.177-1.166.26-1.979.2-.183-.016-.375-.031-.576-.06l-.302-.052h-.008a10.905 10.905 0 01-.642-.14h-.007c-.117-.03-.234-.065-.347-.096a16.097 16.097 0 01-1.521-.502c-.136-.052-.279-.111-.421-.17a20.905 20.905 0 01-.317-.132c.009-.063-.079.096 0 0a.666.666 0 00-.052-.288c1.638-.87 2.78-1.814 3.433-2.665.331-.433.548-.844.635-1.233.087-.386.027-.806-.251-1.077l.007.008.11.351zM13.9 4.947l2.532 2.466.015.015.015.015c.038.038.078.11.03.332-.022.095-.146.264-.207.391-.082.166-.116.3-.274.502-.546.7-1.616 1.566-3.137 2.377.046-.107.09-.213.133-.317.06-.147.115-.294.17-.436.218-.568.4-1.103.538-1.602a11.804 11.804 0 00.244-1.063V7.62c.019-.107.043-.212.059-.324a8.97 8.97 0 00.067-.628c.05-.677-.07-1.196-.185-1.72zM7.774 7.51c1.592.72 2.634 2.424 3.277 3.617-.108-.047-.217-.089-.324-.133-.147-.06-.294-.115-.436-.17a17.265 17.265 0 00-1.595-.539H8.69c-.125-.034-.244-.067-.362-.096-.24-.059-.476-.106-.701-.148-.108-.018-.22-.034-.332-.051V9.98a10.394 10.394 0 00-.628-.067c-.68-.051-1.202.069-1.727.185l2.473-2.532.03-.03c.036-.039.11-.08.332-.03zm-1.167 3.203c.18.012.368.038.569.067h.007c.096.015.194.034.303.052.205.038.418.085.642.14l.177.044c.057.015.113.027.17.044h.007c.47.13.974.299 1.521.51.137.053.28.104.42.162.12.049.24.103.363.155l.014.266c-1.64.87-2.787 1.82-3.44 2.672-.332.433-.548.837-.635 1.225v.008c-.087.387-.027.8.251 1.07H6.97l-.188-.335-5.94 6.071c.04-2.037.114-4.208.458-6.156.372-2.105 1.031-3.928 2.09-5.012v-.008c.024-.027.053-.048.08-.074l.03-.03a1.788 1.788 0 01.089-.081h.007c.006-.004.01-.012.015-.016.171-.125.533-.367.68-.435a3.658 3.658 0 01.331-.133c.519-.18 1.166-.269 1.986-.207zm5.153.2c-.415.132-.698.42-.834.834l.44-.49zm.546.03l.18.323.146.095.405.313c-.148-.348-.372-.603-.73-.73zm-.288.517c.312 0 .554.242.554.553a.547.547 0 01-.554.554.547.547 0 01-.553-.554c0-.31.242-.554.554-.554zm-1.048.863c.14.363.381.61.746.746l-.241-.276-.203-.234zm2.06.038l-.282.116-.062.178-.284.327c.29-.142.498-.324.628-.62zm-.074.51c.106.045.213.096.318.14.147.06.292.115.435.17.566.217 1.098.394 1.595.53.128.039.253.067.369.097.24.059.476.114.701.155.107.019.22.034.332.051.216.03.426.058.628.074.68.052 1.202-.068 1.727-.184l-2.473 2.532-.015.015-.014.015c-.037.038-.115.074-.325.03h-.008c-.216-.05-.531-.204-.893-.487-.7-.547-1.567-1.618-2.377-3.137zm-1.83.088l-.134.317c-.06.147-.115.294-.17.436a17.065 17.065 0 00-.531 1.594c-.036.128-.067.251-.096.37-.06.24-.114.475-.155.7a12.379 12.379 0 00-.052.333 9.86 9.86 0 00-.073.628c-.052.68.068 1.202.184 1.727L7.567 16.59l-.015-.023-.015-.008v-.008c-.04-.04-.078-.101-.03-.317.002-.004 0-.002 0-.008.05-.216.206-.532.488-.893.546-.7 1.613-1.567 3.13-2.377zm1.04.258c.866 1.626 1.81 2.776 2.658 3.426.433.332.836.548 1.225.634h.008c.387.085.799.027 1.07-.25l-.035-.071-.304.27 6.09 5.942c-2.04-.04-4.22-.122-6.172-.467-2.105-.372-3.928-1.03-5.012-2.089h-.008l-.022-.023-.03-.03c-.008-.008-.013-.019-.022-.03l-.03-.03c-.023-.027-.053-.057-.08-.089v-.008c-.003-.004-.012-.01-.015-.015a6.316 6.316 0 01-.436-.679 3.49 3.49 0 01-.133-.332c-.18-.519-.269-1.166-.206-1.986.013-.18.037-.368.066-.568v-.008c.014-.094.033-.196.052-.303.037-.205.085-.419.14-.642v-.008l.044-.17.045-.17v-.008c.13-.47.298-.973.51-1.52.052-.137.111-.28.169-.42.047-.115.097-.231.148-.348l.28-.015z"></path>
                  </g>
                </svg>
              </div>

              {/* Apache Spark Logo */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <svg fill="#534c44" viewBox="0 -1 34 34" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" stroke="#534c44" strokeWidth="0.20400000000000001" width="100%" height="100%">
                  <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                  <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <path fill="#534c44" d="M26.588 15.625c0.53 0.138 1.059 0.276 1.751 0.456-0.34-0.655-0.596-1.212-0.911-1.732-0.195-0.321-0.159-0.526 0.088-0.793 0.461-0.498 0.883-1.032 1.321-1.551-0.021-0.043-0.043-0.086-0.064-0.129-0.539 0.139-1.092 0.238-1.613 0.428-0.549 0.2-0.894 0.125-1.147-0.447-0.201-0.453-0.511-0.858-0.866-1.434-0.134 0.7-0.281 1.246-0.333 1.8-0.053 0.575-0.279 0.891-0.871 1.011-0.557 0.113-1.091 0.34-1.801 0.57 0.696 0.299 1.252 0.537 1.882 0.808-0.381 0.249-0.704 0.468-1.036 0.67-0.085 0.052-0.225 0.104-0.299 0.069-0.616-0.287-1.269-0.527-1.818-0.914-0.519-0.366-0.387-1.031 0.229-1.282 0.731-0.297 1.489-0.533 2.247-0.752 0.349-0.101 0.496-0.272 0.541-0.628 0.075-0.587 0.197-1.169 0.313-1.751 0.038-0.192 0.093-0.388 0.181-0.561 0.351-0.692 0.939-0.741 1.378-0.103 0.326 0.474 0.637 0.964 0.897 1.476 0.215 0.422 0.466 0.5 0.905 0.357 0.665-0.218 1.345-0.409 2.033-0.526 0.293-0.050 0.622 0.115 0.935 0.183-0.084 0.312-0.089 0.682-0.267 0.924-0.468 0.637-1.001 1.228-1.53 1.818-0.223 0.249-0.273 0.449-0.091 0.755 0.33 0.556 0.654 1.125 0.891 1.724 0.11 0.279 0.129 0.728-0.027 0.939-0.156 0.21-0.6 0.332-0.888 0.291-0.581-0.083-1.2-0.209-1.69-0.504-0.276-0.166-0.327-0.705-0.48-1.077 0.047-0.031 0.094-0.063 0.141-0.095z"></path>
                    <path fill="#534c44" d="M9.035 21.264c-0.090 0.62-0.175 1.205-0.265 1.826-0.365 0-0.714 0-1.125 0 0.116-0.934 0.226-1.837 0.341-2.739 0.059-0.464 0.118-0.927 0.191-1.389 0.218-1.363 1.353-2.462 2.666-2.591 1.975-0.193 3.242 1.545 2.458 3.372-0.728 1.697-2.448 2.328-4.268 1.521zM12.317 18.829c0.007-0.748-0.537-1.299-1.286-1.3-0.903-0.002-1.698 0.789-1.695 1.687 0.003 0.734 0.566 1.281 1.318 1.278 0.869-0.003 1.654-0.79 1.663-1.666z"></path>
                    <path fill="#534c44" d="M3.315 20.211c0.404-0.216 0.782-0.418 1.19-0.636 0.116 0.19 0.206 0.362 0.319 0.517 0.322 0.441 0.833 0.495 1.276 0.145 0.41-0.324 0.435-0.637 0.043-1.109-0.333-0.401-0.69-0.784-1.050-1.163-0.546-0.576-0.857-1.229-0.527-2.008 0.357-0.844 1.022-1.325 1.946-1.422 0.879-0.092 1.478 0.32 1.938 1.097-0.363 0.277-0.727 0.553-1.089 0.829-0.183-0.195-0.319-0.467-0.507-0.511-0.294-0.068-0.685-0.082-0.925 0.063-0.354 0.215-0.133 0.533 0.085 0.772 0.339 0.373 0.67 0.756 1.030 1.107 0.608 0.594 0.869 1.288 0.596 2.107-0.296 0.888-0.926 1.453-1.847 1.642-1.306 0.268-2.062-0.168-2.48-1.431z"></path>
                    <path fill="#534c44" d="M18.955 21.507c-0.427 0-0.764 0-1.161 0 0.101-0.776 0.215-1.518 0.289-2.264 0.061-0.617-0.037-1.204-0.621-1.565-0.473-0.292-1.215-0.185-1.716 0.242-0.499 0.425-0.76 0.959-0.582 1.631 0.227 0.857 1.151 1.195 2.008 0.739 0.155-0.083 0.301-0.183 0.499-0.306-0.036 0.249-0.067 0.444-0.092 0.638-0.11 0.857-0.253 0.991-1.1 1.030-1.831 0.086-2.97-1.392-2.425-3.145 0.419-1.346 1.737-2.244 3.149-2.144 1.179 0.084 2.113 1.035 2.065 2.217-0.039 0.967-0.201 1.928-0.313 2.926z"></path>
                    <path fill="#534c44" d="M24.070 17.694c0.738-0.808 1.364-1.494 2.012-2.204 0.284 0.732 0.358 1.349-0.332 1.86-0.311 0.23-0.545 0.564-0.828 0.866 0.778 1.094 1.536 2.162 2.364 3.326-0.525 0-0.964 0.027-1.396-0.016-0.137-0.013-0.285-0.191-0.378-0.327-0.504-0.74-0.992-1.491-1.58-2.381-0.112 0.782-0.256 1.407-0.277 2.036-0.018 0.574-0.242 0.766-0.781 0.681-0.172-0.027-0.35-0.018-0.586-0.029 0.144-1.142 0.28-2.244 0.423-3.345 0.085-0.66 0.217-1.317 0.259-1.979 0.027-0.431 0.191-0.7 0.559-0.896 0.281-0.15 0.542-0.338 0.909-0.571-0.124 1.011-0.237 1.927-0.366 2.978z"></path>
                    <path fill="#534c44" d="M22.251 17.65c-0.212 0-0.355 0.008-0.496-0.001-0.422-0.027-0.627 0.177-0.678 0.588-0.114 0.909-0.28 1.814-0.349 2.726-0.036 0.477-0.198 0.641-0.653 0.575-0.173-0.025-0.352-0.004-0.602-0.004 0.11-0.886 0.21-1.717 0.316-2.547 0.043-0.339 0.090-0.677 0.148-1.013 0.172-1.008 0.748-1.489 1.777-1.493 0.18-0.001 0.36 0.004 0.539 0.011 0.032 0.001 0.064 0.028 0.128 0.057-0.040 0.341-0.081 0.691-0.129 1.102z"></path>
                    <path fill="transparent" d="M12.317 18.829c-0.009 0.876-0.794 1.663-1.663 1.666-0.751 0.003-1.315-0.544-1.318-1.278-0.003-0.898 0.791-1.689 1.695-1.687 0.748 0.001 1.293 0.552 1.286 1.3z"></path>
                  </g>
                </svg>
              </div>

              {/* Hadoop Logo (Elephant) */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="#534c44" stroke="#534c44" strokeWidth="0.168" width="100%" height="100%">
                  <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                  <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
                  <g id="SVGRepo_iconCarrier">
                    <path fill="#534c44" fillRule="evenodd" d="M22.6148786,8.51458809 C22.4527967,9.15577678 22.1254391,9.70110654 21.4473346,9.94260905 C21.1149047,10.060965 20.9000793,9.93918049 20.561027,9.84144289 C20.8938327,9.87309841 21.0852218,9.92424509 21.3832721,9.77672285 C21.939686,9.50159222 22.3496579,9.05202745 22.6148786,8.51458809 L22.6148786,8.51458809 Z M23.9697255,12.7295161 C23.8906337,13.5066732 23.683182,14.2616621 23.3136953,14.9015827 C22.9828153,15.4754212 22.5216967,15.9578157 21.9074669,16.2867231 C21.4900743,16.5086405 21.0050967,16.6371413 20.5037277,16.68965 C19.9832433,16.7437556 19.4459448,16.7148241 18.9445759,16.6297205 C18.9211395,16.7074034 18.8969047,16.7863543 18.8738441,16.8635205 C18.8445369,16.9605536 18.814854,17.0582442 18.7855468,17.1547137 C18.6832065,17.4910419 18.4386981,17.7941647 18.1340254,18.0187592 C17.8195837,18.2507744 17.436195,18.4023358 17.0787318,18.4183984 C16.7372843,18.4349776 16.4734256,18.3545238 16.2394847,18.2163948 C16.0129176,18.0831974 15.8191802,17.897867 15.6078303,17.69375 L15.3309149,17.4273551 C15.268825,17.3668151 15.2066881,17.306463 15.1438468,17.2464866 C15.1371775,17.2900247 15.1309779,17.3316371 15.1243086,17.3696801 C15.2219523,17.5671749 15.2910872,17.7523174 15.3613963,17.9421566 L15.3613963,17.9427202 C15.4057328,18.061264 15.4512435,18.1831425 15.5061475,18.3152127 C15.5879634,18.5105 15.6497245,18.6734273 15.692699,18.8423663 C15.7356265,19.0129022 15.7592977,19.1863031 15.764558,19.3988272 C15.7695834,19.5798836 15.7745149,19.7605643 15.7795403,19.941245 C15.7840491,20.111734 15.7883701,20.2822699 15.7938182,20.4527588 C15.8691997,20.5894787 15.913771,20.6832241 15.9359863,20.7867386 C15.9609725,20.8971103 15.9584833,21.0047109 15.9457553,21.1800843 C15.918045,21.5742754 15.7877595,21.7939384 15.568848,21.9185879 C15.3633689,22.0350182 15.0912911,22.0523959 14.754775,22.050799 C14.7035814,22.0500006 14.6206383,22.0500006 14.5199888,22.0500006 C14.1934766,22.0500006 13.6883973,22.050799 13.533877,22.0398558 C13.3262844,22.0248265 13.153588,22.0089987 13.0086959,21.9742434 C12.8969623,21.9477072 12.8051425,21.9089598 12.728399,21.8527407 C12.6168532,21.9427757 12.5188338,22.0242159 12.4203447,22.1052803 C12.1304196,22.3441527 11.8361266,22.5875339 11.7175827,22.6663909 C11.672166,22.7363242 11.6310702,22.8068212 11.593356,22.8722927 C11.4716654,23.0855682 11.3816303,23.2433292 11.094711,23.3990237 C10.5312052,23.7033206 10.0951198,23.6767844 9.71257649,23.4419981 C9.3481153,23.218343 9.04705907,22.8074318 8.73228859,22.3218905 C8.58255892,22.0914252 8.34349867,21.7117468 8.22711532,21.3394422 C8.0984267,20.9273568 8.11627402,20.5265434 8.5268095,20.3156162 C8.54634763,20.3054714 8.56616756,20.2952796 8.58471938,20.2855106 C8.627506,20.2632484 8.66785035,20.2429588 8.70814774,20.2221995 C8.55254723,20.2304187 8.39614829,20.2367122 8.24054778,20.2347396 C7.93507669,20.232767 7.62814964,20.211726 7.31516389,20.1714286 C7.32023629,20.2087671 7.32455722,20.2386379 7.32695252,20.2579881 C7.35090551,20.4655807 7.36748474,20.6015022 7.34775875,20.7376585 C7.32793882,20.8780888 7.27218941,21.003067 7.15350469,21.1800374 C7.14561429,21.191779 7.11903117,21.2302447 7.09629931,21.2624638 C6.98625645,21.4245927 6.92120764,21.5210622 6.76983413,21.8560283 C6.78476953,22.0211161 6.78965406,22.1538908 6.77871083,22.2861019 C6.76668737,22.4285048 6.73672264,22.5733969 6.68604563,22.7628604 C6.55684038,23.2406052 6.07646549,23.3507419 5.54423942,23.3173487 C5.05588021,23.2869143 4.52121188,23.1331924 4.21231223,23.0492161 C4.14524385,23.0314157 4.09085647,23.0173257 4.03801899,23.0034705 C3.77735409,22.9355098 3.49649353,22.862054 3.26011038,22.7546412 C2.78678049,22.5385947 2.48342291,22.1995894 2.77884313,21.5505103 C2.82642035,21.4476064 2.8678919,21.3429177 2.90560612,21.2374306 C2.92007184,21.1999512 2.92955911,21.1640217 2.9418174,21.1265424 C2.70083153,21.1640217 2.44481632,21.1183232 2.19551735,21.0235915 C1.75938505,20.8577992 1.342603,20.5410091 1.08762107,20.2859333 C0.883738843,20.0815814 0.725132469,19.8617305 0.606447747,19.6359148 C0.478040929,19.3913595 0.395896259,19.1378805 0.354095942,18.8882528 C0.285242834,18.4796899 0.333289716,18.4331929 0.574792225,18.2013185 C0.610815645,18.1659996 0.651159997,18.1275339 0.661022993,18.1179527 C0.780365248,17.9999725 0.89890907,17.8816635 1.01796953,17.7642469 C1.14501431,17.6382824 1.27351506,17.5113315 1.40009018,17.3847564 C1.45828186,17.2273242 1.51619174,17.0703147 1.57457128,16.9115204 C1.61209764,16.8101664 1.64929522,16.7080139 1.68639887,16.6070826 C1.66855155,16.5129145 1.65239502,16.4182297 1.63882166,16.3213375 C1.6231348,16.2082418 1.61078257,16.1043516 1.60261037,16.0106062 C1.56588245,15.5787948 1.55357719,15.2074295 1.56639909,14.8711483 C1.57607422,14.6146634 1.59998024,14.37875 1.63933829,14.1529813 C1.4399179,14.1974117 1.23678714,14.1939832 1.05272485,14.1356036 C0.825265365,14.0621478 0.626737339,13.9051383 0.503778652,13.6470096 C0.457704369,13.5522778 0.421446116,13.4564659 0.387160463,13.3483017 C0.357007874,13.2528654 0.329156651,13.1463451 0.299285862,13.0256408 C0.13617069,12.8575941 0.0437403251,12.6812344 0.0130711032,12.5006476 C-0.0233750164,12.2933838 0.0186131678,12.0866835 0.111419266,11.8886252 C0.200280166,11.6982694 0.336718282,11.5172129 0.494620156,11.3529706 C0.919903162,10.9112492 1.5244109,10.579242 1.80574113,10.5303967 L2.01676228,10.4934809 L1.94654714,10.6956253 C1.91005406,10.7979657 1.86736137,10.9051905 1.82607769,11.0107246 C1.81442996,11.0413938 1.80264133,11.0723448 1.7898664,11.1056442 C1.91925952,11.2079845 1.99680146,11.3499647 2.03301274,11.5115769 C2.08956059,11.7614865 2.05302054,12.049392 2.00478579,12.264828 L1.94725164,12.5227219 L1.7838077,12.3159747 C1.74426179,12.2672233 1.71133817,12.2164524 1.67968264,12.1672314 C1.65046939,12.1222843 1.62275906,12.0800143 1.59443818,12.045259 C1.57194115,12.3807417 1.53112713,12.7171638 1.29206689,12.946502 C1.29883009,12.9753395 1.30202382,12.9907446 1.30681442,12.9940792 C1.31061872,12.9960518 1.35124487,12.9738835 1.44931123,12.9356997 C1.6176397,12.8714493 1.76736938,12.7642244 1.90822236,12.6381659 C2.05302054,12.5077865 2.18546649,12.3589492 2.31946234,12.2116618 C2.47092978,11.9175097 2.64428368,11.6392792 2.84670994,11.3746691 C3.05247083,11.1046579 3.28796161,10.8493941 3.55980457,10.6085492 C4.22513412,10.0193056 4.87238151,9.62689924 5.6070808,9.34261011 C6.32623413,9.06470845 7.12795483,8.88956982 8.11284546,8.73166794 C8.23679045,8.60316719 8.36256713,8.47588757 8.49153755,8.35001695 C8.6299013,8.21719527 8.77286778,8.08437358 8.92259746,7.95159887 C9.13296108,7.7666442 9.30875724,7.65354851 9.48361408,7.58356821 C9.64151595,7.51931783 9.79730433,7.49419067 9.97239599,7.48141574 C10.2730765,7.0805554 10.5472208,6.73661862 10.8697408,6.4386622 C11.207431,6.12638095 11.5952346,5.86646752 12.1173629,5.6469454 C13.1987699,5.19080531 14.1156059,4.95320103 14.9827981,5.00768234 C15.8562369,5.06230455 16.6734096,5.41197126 17.5491967,6.13455315 C17.7144724,6.2710852 17.8828008,6.41856048 18.0505657,6.56589486 C18.4175631,6.88855573 18.7777034,7.20450038 19.2007789,7.46911048 C19.3601368,7.56783438 19.494931,7.65359548 19.6281285,7.75457378 C19.759776,7.8550824 19.8812787,7.96737966 20.0078069,8.11678057 C20.1607303,8.29985657 20.3003621,8.48269774 20.4228511,8.67850169 C20.5257551,8.84208653 20.6156023,9.01572223 20.6949289,9.20621896 C20.8742006,9.13393729 21.0640398,9.05428185 21.2160239,8.92582807 C21.3472957,8.81324901 21.456634,8.65337454 21.5656436,8.4940167 C21.6632873,8.35053358 21.7613067,8.20836554 21.877737,8.09118375 C21.9156861,8.0518257 21.9589893,8.01970051 22.005909,7.99607628 C22.1844292,7.90181422 22.4103858,7.91736018 22.6242719,7.99119175 C22.8262285,8.06032666 23.0211401,8.18319141 23.153586,8.31385263 C23.2145018,8.37335938 23.2633471,8.43709312 23.2953784,8.49890123 C23.5613975,9.00750307 23.7527865,9.71463294 23.8684184,10.4424281 C23.9972949,11.2597887 24.032426,12.1066913 23.9697255,12.7295161 L23.9697255,12.7295161 Z M20.2449884,9.38187423 C20.2063349,9.38286053 20.1684328,9.38384683 20.1317049,9.38520886 C20.1867968,9.43931444 20.2299591,9.50356482 20.2616146,9.57457839 C20.2772076,9.559643 20.2950079,9.54517727 20.3141233,9.53212054 C20.2914384,9.48139656 20.2684248,9.43109528 20.2449884,9.38187423 L20.2449884,9.38187423 Z M1.49914284,11.6538859 C1.59749101,11.6428487 1.65864158,11.694512 1.73472756,11.7702692 C1.72096633,11.5887431 1.68658674,11.4539958 1.55911926,11.379084 C1.54996076,11.4085791 1.5427279,11.438262 1.53422693,11.4682267 C1.5177886,11.5266063 1.50764381,11.5886962 1.49914284,11.6538859 L1.49914284,11.6538859 Z M1.88995233,13.6347043 C1.89915779,13.6020625 1.90878596,13.5699373 1.91935345,13.5371076 C1.98116156,13.3420081 2.04738454,13.1526386 2.11839811,12.969046 C1.9698426,13.1033236 1.80278223,13.2172647 1.60012114,13.2984231 C1.07606727,13.5080822 1.0867287,13.3269318 0.861147885,12.8719659 C1.34072434,12.4889529 1.11875995,12.0185349 1.27140156,11.4898314 C1.30695532,11.3683287 1.35843076,11.249597 1.43127604,11.0930571 C0.929014692,11.3954284 0.00597913924,12.2399357 0.622792141,12.8799502 C0.689672649,13.1214528 0.741335963,13.3093663 0.833155762,13.5003327 C1.02196169,13.8947586 1.51515847,13.7935925 1.88995233,13.6347043 L1.88995233,13.6347043 Z M3.27852131,20.0330179 C2.74235004,19.2215281 2.25798299,18.2554711 1.97374083,17.2730697 C1.92127909,17.4138757 1.86867644,17.5535545 1.81649649,17.6941727 C1.57616815,17.9367084 1.33593374,18.1800896 1.0956054,18.4236117 C0.91755483,18.6042924 0.88279951,18.610539 0.91882293,18.8658027 C0.967386445,19.2162209 1.17413363,19.5886664 1.48298632,19.9039065 C1.76549071,20.1924227 2.74070621,20.9449223 3.1241889,20.4320465 C3.20581694,20.3226612 3.24423566,20.1874912 3.27852131,20.0480472 L3.27852131,20.0330179 L3.27852131,20.0330179 Z M15.0621717,18.4968797 C14.9773969,18.2953458 14.9305242,18.106305 14.8715341,17.9154326 C14.7439727,18.3717136 14.5718869,18.8011766 14.4156758,19.294937 C14.2676369,19.7621142 13.507059,21.1168203 13.0602183,21.4461034 C13.1472945,21.5109644 13.307075,21.5359506 13.5918338,21.5606081 C13.7934147,21.5777509 14.5869632,21.5824476 14.7869472,21.5853595 C15.2259914,21.5933908 15.3926291,21.5599976 15.4371535,21.0603663 C15.4586172,20.8152474 15.4390791,20.7697367 15.316778,20.5550991 C15.3062574,20.1726497 15.2957369,19.7894488 15.2851694,19.4070464 C15.2740853,19.0447456 15.2043868,18.8351804 15.0621717,18.4968797 L15.0621717,18.4968797 Z M22.9516294,8.87327239 C22.849477,8.62589905 22.441055,8.25519129 22.1816582,8.50716736 C21.91996,8.7621493 21.7230758,9.12102146 21.4379413,9.34660228 C21.0492923,9.65207336 20.3809099,9.54700897 20.5019899,10.196229 C20.5904751,10.6690892 20.6156492,11.1983563 20.5144831,11.6389035 C20.4180136,12.0620261 20.3176458,12.6633401 20.0969026,12.9465959 C20.1621393,12.7068781 20.2719003,12.0801082 20.3025695,11.6572675 C20.3191957,11.4259098 20.2932232,11.146693 20.2500139,10.8656916 C20.2012155,10.8666779 20.152605,10.8678051 20.1027734,10.869308 C19.9246289,10.8729244 19.634375,11.128376 19.5562695,11.2877808 C19.3382973,11.7317096 19.3234558,12.1442177 19.0793232,12.5667766 C19.2758316,12.094433 19.1652721,11.6684455 19.3447317,11.1618163 C19.4078549,10.9831082 19.568199,10.8290106 19.7535294,10.7352182 C19.6503907,10.7198132 19.5472519,10.7649011 19.3948921,10.8004549 C18.742948,10.9555388 18.7722552,11.4679449 18.3713009,11.9572904 C18.7577895,11.248188 18.5605296,10.7780048 19.3986025,10.5867097 C19.6743906,10.5241971 19.8757366,10.4991169 20.0570279,10.644009 C20.0609261,10.644009 20.0648713,10.644009 20.0683469,10.6432106 C20.116018,10.6395941 20.1632665,10.6356959 20.2113133,10.6323613 C20.1875013,10.5022167 20.1620923,10.3739977 20.1367303,10.2531525 C19.7535294,10.2029452 19.4003872,10.2136066 19.0074172,10.2729725 C19.0923799,10.2360567 19.1749942,10.2019589 19.2550724,10.1729805 C19.3800506,10.1270941 19.5015533,10.0908358 19.6223985,10.0701705 C19.6343281,10.0462644 19.6413261,10.0198692 19.6447077,9.99145437 C19.6572008,9.85905538 19.5591814,9.74079336 19.4269233,9.72768967 C19.2941017,9.71510261 19.1812408,9.81274627 19.168325,9.94556795 C19.1636283,9.99770093 19.1819923,10.0600257 19.2112995,10.111689 C19.2048181,10.1000413 19.1985715,10.0885344 19.1928885,10.0762292 L19.1923249,10.0754307 C19.1417418,9.97788101 19.1186812,9.86534892 19.1290139,9.74868376 C19.1381724,9.66062129 19.1656948,9.57941596 19.20773,9.50746305 C19.1948611,9.51056285 19.1819453,9.51469591 19.1690764,9.51925168 C19.0759416,9.55663713 18.8772726,9.54466064 18.7874254,9.51389748 C18.9635973,9.48468423 19.2260939,9.39103273 19.3956436,9.30841839 C19.4003402,9.30644579 19.404802,9.30452016 19.4085124,9.30217183 C19.5143752,9.23693515 19.6412791,9.20480996 19.7737251,9.21711523 C19.8409344,9.22383146 19.9041515,9.24120912 19.9620144,9.26877854 C20.0409183,9.29202703 20.1292156,9.32340075 20.2354072,9.36158464 C20.0740768,9.02445803 19.8918932,8.71969145 19.6725589,8.41393856 C19.4651542,8.1253285 19.2909079,8.02566527 18.9842627,7.84451491 C18.9174761,7.80487506 18.8522394,7.76528217 18.7878012,7.72371669 C18.5612341,7.68219817 18.3085065,7.64138415 18.0904873,7.68290267 C18.2701817,7.61221786 18.4053518,7.58370911 18.562784,7.57069934 C18.1288121,7.25954529 17.754394,6.91034826 17.3260582,6.55011406 C15.6731609,5.16027698 14.3356447,5.25900088 12.2307873,6.17287794 C11.3766049,6.54475986 11.070852,6.91866136 10.5453421,7.5914586 C10.5499449,7.5914586 10.5554869,7.59098894 10.5617805,7.59098894 C11.096214,7.57750951 11.6275946,7.55914555 12.1770105,7.50748223 C11.608761,7.63978729 11.0654038,7.74555618 10.507487,7.83004918 C9.89184819,7.92398248 9.70280743,7.90834258 9.23844817,8.36025568 C8.38276279,9.19269257 7.66600476,10.1335754 6.88278892,10.977801 C6.42956076,11.4670525 6.20416781,11.9573844 5.93805478,12.5147376 C5.67410221,13.0692258 5.71064226,13.2792607 6.06481776,13.7584614 C6.42664883,14.2489811 6.64537251,14.4708516 6.79876559,14.9063733 C7.19887447,14.2888088 7.68089319,13.7767784 8.1891193,13.1629243 C7.77416895,13.8279721 7.39575866,14.4563858 7.05280819,15.1325647 C6.8391569,15.5532919 6.73423341,15.7676947 6.74109054,16.2329933 C7.14213876,16.6950981 7.39364516,16.9415791 7.76599676,17.0439194 C8.16709194,17.1541032 8.54507953,17.1357392 8.90761509,16.9478257 C9.80956262,16.4810711 10.6668918,15.8720546 11.667939,15.7953111 C12.184713,14.8742481 12.0395391,13.7160036 11.8540678,12.6683655 C11.7004399,11.7977447 11.7394222,11.0299809 11.946827,10.15673 C11.9987721,11.0048538 12.0666389,11.7923905 12.2348734,12.6238881 C12.4799924,13.8419681 12.5315617,14.8935044 12.0192965,16.1521167 C10.9129501,16.1786528 10.0049439,16.8268865 9.02371665,17.3286782 C8.58251195,17.5544939 8.169816,17.5771318 7.67614956,17.4597152 C7.17285495,17.3399972 6.78697696,16.9517239 6.26912269,16.3846955 C6.31989363,15.955984 6.37583091,15.6895891 6.52847252,15.3747717 C6.34379966,14.8184987 6.11479028,14.5626714 5.69147987,13.9937174 C5.20053749,13.3341647 5.19208349,13.0484196 5.56344878,12.297329 C5.83111172,11.7527976 6.08200756,11.2504424 6.50536493,10.7536761 C6.92214697,10.2615126 7.3054418,9.80339991 7.69286968,9.3703674 C6.08271206,9.61478184 5.05489391,10.0030551 3.95516982,10.9676092 C3.13982879,11.6834279 2.63127392,12.5722248 2.26845656,13.5985869 C2.03855481,14.246257 1.97411657,14.8987647 2.08251559,15.9575339 C2.22186564,17.3221968 2.94656103,18.8618575 3.72695887,19.984877 C3.63175748,20.6841165 3.50832912,21.2108945 3.3131827,21.7106666 C3.09196978,22.2774601 3.97226568,22.4509079 4.39754869,22.5672913 C4.74289446,22.662023 6.0544851,23.0219754 6.18416002,22.5571465 C6.26142016,22.2785873 6.26142016,22.0840984 6.2420699,21.790322 C6.47586987,21.324131 6.54688345,21.2618063 6.71375595,20.9860651 C6.89227618,20.6909267 6.90721158,20.5731343 6.90866755,20.2264735 C6.90937204,19.779398 6.89781825,18.9809649 6.91665188,18.6923079 C6.97719189,18.9504836 7.04919176,19.3098724 7.1088394,19.6547954 C8.04108042,19.821574 8.96627644,19.8094566 9.89790689,19.6192417 C9.90974248,19.5541929 9.93130018,19.4832732 9.95544103,19.3961971 C10.0177188,19.1781779 10.142838,18.9602526 10.2058202,18.7418577 C10.1882077,18.956918 10.1718164,19.1713677 10.1543448,19.3858174 C10.1369671,19.6071243 10.1362626,19.7856445 10.1635033,20.0063408 C10.1785326,20.1277966 10.1939846,20.2489235 10.2084504,20.3701914 C10.1410532,20.2590683 10.0360828,20.1524071 9.97474433,20.0406734 C9.55979398,20.3136906 9.33158303,20.4682578 8.92414735,20.6832711 C8.49374498,20.9110124 8.98741143,21.7258368 9.16513323,22.0090927 C9.55584878,22.6291933 10.0224624,23.3643153 10.7482381,22.980363 C10.986312,22.8547741 11.1256621,22.563393 11.2845502,22.353593 C11.497544,22.228239 12.62836,21.27491 12.8080544,21.1709728 C13.0615804,21.0254701 13.9750817,19.5287839 14.0908545,19.15554 C14.3463061,18.3289739 14.646329,17.6999026 14.7311038,16.8375949 C14.1494689,16.5930396 13.8666827,16.3213845 13.4498537,15.8313345 C14.0358096,16.265682 14.5416404,16.4978852 15.1732948,16.6979161 C15.4035254,16.900812 15.6358224,17.1059624 15.8773719,17.3209757 C16.2275552,17.6328812 16.5490889,17.9508454 17.0568453,17.9600039 C17.6060733,17.9693972 18.2271603,17.5494684 18.3869408,17.0287492 C18.4482793,16.8285773 18.5101344,16.6277949 18.5712849,16.4273882 C18.462698,16.4011808 18.3560837,16.3719206 18.2533207,16.3404529 C18.1394266,16.6142685 18.1484442,16.7203662 18.0947143,16.9215243 C18.0021431,17.265649 17.5284844,17.5045213 17.0322348,17.3847564 C16.8558281,17.3422046 16.7521257,17.3175941 16.6892374,17.2722713 C16.7388811,17.407817 16.8361021,17.523261 17.0165949,17.5562316 C17.2166259,17.5933353 17.364242,17.6093979 17.6548717,17.5433628 C17.2955298,17.7297264 17.179898,17.7377108 16.9251978,17.6808811 C16.2550777,17.5320438 16.4910381,16.7250159 16.6371044,16.2314903 C16.7288772,15.920759 16.6978323,15.5910531 16.6282747,15.2797112 C16.8821764,15.4402432 17.0833345,15.5816598 17.3722264,15.6793504 C18.7255234,16.1352087 20.3314071,16.5998498 21.6935338,15.8388961 C22.7618842,15.2426075 23.3476522,13.9201206 23.4792528,12.7087098 C23.5965285,11.6287587 23.3941962,9.94411199 22.9516294,8.87327239 L22.9516294,8.87327239 Z M16.0434459,15.3758049 C16.0168628,15.5439925 15.9807455,15.7515851 15.9541623,15.9197727 C16.0256456,15.7299335 16.1106083,15.5101765 16.1932696,15.3289322 C16.2809563,15.1373553 16.3299425,15.1199776 16.5149441,15.0187645 C16.6470144,14.9469055 16.8860276,14.8484634 17.0174873,14.7773559 C16.8821294,14.7999938 16.6393588,14.8494027 16.5045645,14.8722755 C16.1430152,14.9334261 16.1000408,15.0199386 16.0434459,15.3758049 L16.0434459,15.3758049 Z M9.86540597,8.90807468 C9.44618166,9.32340075 9.03874598,10.7473356 8.90855443,11.3167593 C9.11314115,10.8429127 9.62709719,9.52000315 10.0206777,9.18315834 C10.1293585,9.08988257 10.2059142,9.03267719 10.2868847,8.99106474 C10.0050848,9.46448857 10.0255622,9.58176429 10.1245679,10.2151096 C10.2085443,9.57124376 10.4319647,9.32194479 10.7968955,8.84218047 C11.1974741,8.74092037 11.5732542,8.62115542 11.9837897,8.46081128 C11.5201349,8.51346089 11.0579362,8.56099114 10.593436,8.60476406 C10.2045052,8.64144501 10.1417108,8.63590294 9.86540597,8.90807468 L9.86540597,8.90807468 Z M16.8285874,10.4872343 C16.7385524,10.2980996 16.5803687,10.1626478 16.3973396,10.0959082 C16.5639304,10.006061 16.7266228,9.9133958 16.8543251,9.79766997 C16.4808933,9.97064814 16.0465457,9.92814333 15.7269847,10.1449414 C15.4437758,10.3347806 15.0545632,10.9391004 14.7680196,11.194552 C14.9754243,11.1140982 15.1754083,10.9747012 15.3594237,10.828447 C15.3607857,10.9371278 15.3852084,11.048157 15.4340537,11.1529866 C15.4852004,11.2597887 15.5594077,11.3477103 15.6457793,11.4172209 C15.6211688,11.3926104 15.5998459,11.3645713 15.5840652,11.3319765 C15.5075095,11.1718202 15.575846,10.979163 15.7360023,10.9033588 C15.8965343,10.8260517 16.0900838,10.8945761 16.1666395,11.0545445 C16.1734966,11.0699965 16.1801189,11.0854016 16.1844398,11.1008536 C16.0309059,11.2154052 15.8902877,11.3492132 15.7726832,11.5041562 C15.6699672,11.6393732 15.5840182,11.7920148 15.5227267,11.9610947 C16.0684322,11.3139413 16.8600551,10.8243609 17.6043356,10.6159699 C17.3961324,10.6135276 17.1496514,10.6526039 16.8988495,10.7342319 C16.8898789,10.6514297 16.8670531,10.5676412 16.8285874,10.4872343 L16.8285874,10.4872343 Z M14.1629483,9.9039555 C14.2885371,9.34810521 14.5118166,8.81235664 15.3754393,8.39750023 C14.230909,8.68413769 14.0160836,9.16479438 14.1629483,9.9039555 L14.1629483,9.9039555 Z"></path>
                  </g>
                </svg>
              </div>
            </div>

            {/* Content - top section */}
            <div className="relative z-10">
              <h2
                className="text-[2.75rem] font-bold leading-tight mb-3 max-w-[550px]"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                {card4ScrambledText || card4FinalText}
              </h2>
              <p
                className="text-base font-medium mb-8 max-w-[550px]"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-secondary)' }}
              >
                Petabyte-scale architectural transformation and first-party migration.
              </p>

              {/* Key Deliverables */}
              <div className="space-y-3 max-w-[600px]">
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  1P Data Migration & Integration: 25+ first-party service pipelines with bulk validation
                </div>
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  Data Lake Rearchitecture: Lake Formation with secure tagging & self-service analytics
                </div>
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  Development Catalogs: PII-redacted prod-grade data for lower environments
                </div>
              </div>
            </div>

            {/* Content - metrics section */}
            <div className="relative z-10 mt-auto" style={{ paddingTop: '165px' }}>
              <div
                className="text-[4.5rem] font-mono font-bold leading-none"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                  marginBottom: '16px',
                } : { color: 'var(--text-accent)', marginBottom: '16px' }}
              >
                $870k+
              </div>
              <div
                className="text-xs uppercase tracking-widest mb-14"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-secondary)' }}
              >
                Annualized Infrastructure Cost Savings
              </div>

              <div
                className="text-sm flex items-center gap-2"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                View Case Study
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M 6 3 L 6 5 L 17.59375 5 L 3.75 18.84375 L 5.15625 20.25 L 19 6.40625 L 19 18 L 21 18 L 21 3 Z"/>
                </svg>
              </div>
            </div>
          </div>
        </Card>

        {/* Card 5: Business Intelligence */}
        <Card href="/business-intelligence" isOutlineMode={isOutlineMode}>
          <div ref={card5Ref} className="h-full flex flex-col relative overflow-hidden" style={{ padding: '60px' }}>
            {/* Tech Stack Logos - vertically stacked, right aligned */}
            <div
              className="absolute flex flex-col gap-6 items-end"
              style={{
                right: '80px',
                top: '50%',
                transform: 'translateY(-50%)',
                paddingRight: '16px',
              }}
            >
              {/* BI Icons: use public SVGs (tableau, SQL, Python, RLang) */}
              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <img src="/tableau.svg" alt="Tableau" style={{ width: '100%', height: '100%' }} />
              </div>

              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <img src="/Sql.svg" alt="SQL" style={{ width: '100%', height: '100%' }} />
              </div>

              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <img src="/Python.svg" alt="Python" style={{ width: '100%', height: '100%' }} />
              </div>

              <div
                style={{
                  width: '100px',
                  height: '100px',
                  padding: '8px',
                  opacity: isOutlineMode ? 0.6 : 0.4,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                }}
              >
                <img src="/RLang.svg" alt="R" style={{ width: '100%', height: '100%' }} />
              </div>
            </div>

            {/* Content - top section */}
            <div className="relative z-10">
              <h2
                className="text-[2.75rem] font-bold leading-tight mb-3 max-w-[550px]"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                {card5ScrambledText || card5FinalText}
              </h2>

              <p
                className="text-base font-medium mb-8 max-w-[550px]"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-secondary)' }}
              >
                VP Metrics Automation & Data Governance Framework
              </p>

              {/* Key Deliverables */}
              <div className="space-y-3 max-w-[600px]">
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  VP Executive Metrics: Automated 400+ KPI consolidation for executive velocity
                </div>
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  Tableau & Redshift Architecture: Global dashboards with standardized patterns
                </div>
                <div
                  className="text-sm"
                  style={isOutlineMode ? {
                    color: '#D5CBBA',
                    textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                    WebkitFontSmoothing: 'antialiased',
                  } : { color: 'var(--text-primary)' }}
                >
                  BI Governance: CI/CD processes and standards framework for data products
                </div>
              </div>
            </div>

            {/* Content - metrics section */}
            <div className="relative z-10 mt-auto" style={{ paddingTop: '165px' }}>
              <div
                className="text-[4.5rem] font-mono font-bold leading-none"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                  marginBottom: '16px',
                } : { color: 'var(--text-accent)', marginBottom: '16px' }}
              >
                $1.5M+
              </div>
              <div
                className="text-xs uppercase tracking-widest mb-14"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-secondary)' }}
              >
                OPEX Reclaimed
              </div>

              <div
                className="text-sm flex items-center gap-2"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                View Case Study
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M 6 3 L 6 5 L 17.59375 5 L 3.75 18.84375 L 5.15625 20.25 L 19 6.40625 L 19 18 L 21 18 L 21 3 Z"/>
                </svg>
              </div>
            </div>
          </div>
        </Card>

        {/* Card 6: About */}
        <Card isOutlineMode={isOutlineMode}>
          <div ref={card6Ref} className="h-full flex flex-col relative" style={{ padding: '60px' }}>
            {/* Top Section */}
            <div>
              <h2
                className="text-[2.75rem] font-bold leading-tight mb-2"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                  WebkitFontSmoothing: 'antialiased',
                } : { color: 'var(--text-primary)' }}
              >
                {card6ScrambledText || card6FinalText}
              </h2>

              <p
                className="text-base font-medium mb-8 leading-relaxed"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44`,
                } : { color: 'var(--text-secondary)' }}
              >
                Building AI systems at AWS T&C
              </p>

              <p
                className="text-base font-medium mb-7 leading-relaxed"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44`,
                } : { color: 'var(--text-primary)' }}
              >
                <strong>12+ years</strong> at Amazon. Started in fulfillment operations, built startups, now building AI infrastructure for <strong>210+ engineers</strong>.
              </p>

              <p
                className="text-base font-medium leading-relaxed"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44`,
                } : { color: 'var(--text-primary)' }}
              >
                <strong>Build</strong> tools, deployment systems, data platforms, and ML infrastructure. Enterprise and startup scale.
              </p>
            </div>

            {/* Bottom Section */}
            <div className="relative z-10 mt-auto" style={{ paddingTop: '80px' }}>
              <p
                className="text-base font-medium mb-7 leading-relaxed"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44`,
                } : { color: 'var(--text-primary)' }}
              >
                Environmental safety → BI → Data engineering → ML → AI. <em style={{ fontFamily: 'Georgia, serif' }}>Risk assessment still applies.</em>
              </p>

              <p
                className="text-base font-medium leading-relaxed"
                style={isOutlineMode ? {
                  color: '#D5CBBA',
                  textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44`,
                } : { color: 'var(--text-primary)' }}
              >
                Golden Gloves champion turned systems builder. <em style={{ fontFamily: 'Georgia, serif' }}>Same discipline, different ring.</em>
              </p>
            </div>

            {/* AWS Certifications */}
            <div className="relative z-10 mt-auto" style={{ paddingTop: '30px' }}>
              <div className="flex gap-4 justify-center">
                <img src="/genai-badge.png" alt="AWS Certified Generative AI Developer" style={{ height: '102px', width: 'auto' }} />
                <img src="/solutions-architect-badge.png" alt="AWS Certified Solutions Architect" style={{ height: '102px', width: 'auto' }} />
                <img src="/data-engineer-badge.png" alt="AWS Certified Data Engineer" style={{ height: '102px', width: 'auto' }} />
                <img src="/ml-engineer-badge.png" alt="AWS Certified Machine Learning Engineer" style={{ height: '102px', width: 'auto' }} />
              </div>
            </div>
          </div>
        </Card>

        {/* Card 7: Contact Form */}
        <Card isOutlineMode={isOutlineMode}>
          <div ref={card7Ref} className="h-full flex flex-col" style={{ padding: '60px' }}>
            <h2
              className="text-[2.75rem] font-bold leading-tight mb-2"
              style={isOutlineMode ? {
                color: '#D5CBBA',
                textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44, 0 -1px 0 #534C44, 0 1px 0 #534C44, -1px 0 0 #534C44, 1px 0 0 #534C44`,
                WebkitFontSmoothing: 'antialiased',
              } : { color: 'var(--text-primary)' }}
            >
              {card7ScrambledText || card7FinalText}
            </h2>
            <p
              className="text-base font-medium mb-8 leading-relaxed"
              style={isOutlineMode ? {
                color: '#D5CBBA',
                textShadow: `-1px -1px 0 #534C44, 1px -1px 0 #534C44, -1px 1px 0 #534C44, 1px 1px 0 #534C44`,
              } : { color: 'var(--text-secondary)' }}
            >
              Get in touch for opportunities or collaboration.
            </p>

            <form onSubmit={handleContactSubmit} className="flex flex-col gap-4 flex-1">
              {/* Top spacer to center + sign */}
              <div className="flex-1"></div>

              <input
                type="text"
                placeholder="Name"
                value={formName}
                onChange={(e) => setFormName(e.target.value)}
                required
                className="w-full py-6 text-base font-mono bg-transparent border-2 focus:outline-none transition-colors"
                style={isOutlineMode ? {
                  borderColor: '#534C44',
                  color: '#D5CBBA',
                  paddingLeft: '12px',
                  paddingRight: '12px',
                } : {
                  borderColor: '#534C44',
                  color: 'var(--text-primary)',
                  paddingLeft: '12px',
                  paddingRight: '12px',
                }}
              />

              <input
                type="email"
                placeholder="Email"
                value={formEmail}
                onChange={(e) => setFormEmail(e.target.value)}
                required
                className="w-full py-6 text-base font-mono bg-transparent border-2 focus:outline-none transition-colors"
                style={isOutlineMode ? {
                  borderColor: '#534C44',
                  color: '#D5CBBA',
                  paddingLeft: '12px',
                  paddingRight: '12px',
                } : {
                  borderColor: '#534C44',
                  color: 'var(--text-primary)',
                  paddingLeft: '12px',
                  paddingRight: '12px',
                }}
              />

              {/* Spacer to push message below + sign */}
              <div className="flex-1"></div>

              <textarea
                placeholder="Message"
                value={formMessage}
                onChange={(e) => setFormMessage(e.target.value)}
                required
                rows={4}
                className="w-full py-6 text-base font-mono bg-transparent border-2 focus:outline-none resize-none transition-colors"
                style={isOutlineMode ? {
                  borderColor: '#534C44',
                  color: '#D5CBBA',
                  paddingLeft: '12px',
                  paddingRight: '12px',
                } : {
                  borderColor: '#534C44',
                  color: 'var(--text-primary)',
                  paddingLeft: '12px',
                  paddingRight: '12px',
                }}
              />

              <button
                type="submit"
                disabled={formLoading}
                className="px-20 py-5 text-base font-mono border-2 hover:shadow-lg transition-all self-start"
                style={{
                  borderColor: '#534C44',
                  color: '#D5CBBA',
                  backgroundColor: '#534C44',
                }}
              >
                {formLoading ? 'Sending...' : 'Send Message'}
              </button>

              {formSuccess && (
                <p className="text-sm" style={{ color: 'var(--text-accent)' }}>
                  Message sent successfully!
                </p>
              )}

              {formError && (
                <p className="text-sm" style={{ color: '#8D152C' }}>
                  {formError}
                </p>
              )}

              {/* Social/Contact Icons */}
              <div className="flex gap-6 mt-auto pt-8">
                {/* Resume Download */}
                <a
                  href="/resume.pdf"
                  download
                  className="hover:opacity-70 transition-opacity"
                  aria-label="Download Resume"
                  style={{ color: '#534C44' }}
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                    <polyline points="7 10 12 15 17 10"></polyline>
                    <line x1="12" y1="15" x2="12" y2="3"></line>
                  </svg>
                </a>

                {/* LinkedIn */}
                <a
                  href="https://linkedin.com/in/yourusername"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:opacity-70 transition-opacity"
                  aria-label="LinkedIn Profile"
                  style={{ color: '#534C44' }}
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                  </svg>
                </a>

                {/* Calendly */}
                <a
                  href="https://calendly.com/yourusername"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:opacity-70 transition-opacity"
                  aria-label="Schedule Meeting"
                  style={{ color: '#534C44' }}
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
                    <line x1="16" y1="2" x2="16" y2="6"></line>
                    <line x1="8" y1="2" x2="8" y2="6"></line>
                    <line x1="3" y1="10" x2="21" y2="10"></line>
                  </svg>
                </a>
              </div>
            </form>
          </div>
        </Card>
      </HorizontalScroll>

      {/* Theme toggle plus sign - centered on page */}
      <div
        className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 cursor-pointer transition-opacity hover:opacity-70"
        onClick={() => setIsOutlineMode(!isOutlineMode)}
        style={{
          width: '20px',
          height: '20px',
        }}
      >
        {/* Horizontal line */}
        <div
          style={{
            position: 'absolute',
            width: '20px',
            height: '1px',
            backgroundColor: 'var(--text-primary)',
            top: '50%',
            left: '0',
            transform: 'translateY(-50%)',
          }}
        />
        {/* Vertical line */}
        <div
          style={{
            position: 'absolute',
            width: '1px',
            height: '20px',
            backgroundColor: 'var(--text-primary)',
            left: '50%',
            top: '0',
            transform: 'translateX(-50%)',
          }}
        />
      </div>

      {/* Progress indicator */}
      <ProgressIndicator progress={scrollProgress} />
    </main>
  );
}
